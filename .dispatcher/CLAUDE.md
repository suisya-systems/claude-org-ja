...
- pane_output_without_peer_msg 検出 (Step 5.2) は worker がペインに出力したのに secretary 宛 `send_message` を発行し忘れた **silent dead-lock** を捕捉する。`PANE_OUTPUT_SETTLE_SEC = 60` 秒静止 + `idle_streak_cycles >= 1` + 過去 `PANE_OUTPUT_LOOKBACK_MIN = 15` 分以内に worker→secretary 痕跡なし、で fire し worker への self-healing nudge と secretary への informational を両系統送信する。詳細は [`.dispatcher/references/worker-monitoring.md` Step 5.2](references/worker-monitoring.md#step-5-2)
