...
- **`--max-concurrent-workers N`（broker 面で load-bearing）**: broker 面では helper が rect geometry の `choose_split` を**バイパス**し、「アクティブ worker 数 < `N` なら固定 spawn target で spawn / 到達で `split_capacity_exceeded`」を返す。既定 8（`registry/org-config.md` の `max_concurrent_workers` が運用値の導線）、`unlimited` は opt-in。**renga 面で無視されるのは caller タブ内に配置する既定経路のときだけ**（renga は MIN_PANE まで分割し続ける rect ベース balanced split が律速するため）で、`--transport renga` の現行運用では省略しても挙動は変わらない。ただし **`--overflow-to-new-tab` を armed にすると renga 面でも fleet ceiling が live になる**（runtime 0.1.39 以降）: rect 上限が消える代わりに `max_concurrent_workers` が唯一の上限になり、`--peers-json` が**必須**になる（欠落は `input_invalid` / exit 1。overflow で置いたワーカーは自分のタブに居て `--panes-json` に二度と現れないため、census 無しでは上限が 0 のままタブを無制限に増やす）。**現行 ja はこのフラグを渡していない**（repo 全体で出現 0 件）ので今日は到達しない。正準条件は runtime CLI の `ceiling_applies`（`transport == "broker" or (transport == "renga" and --overflow-to-new-tab and --tab 未指定)`）。
...
- **exit 2 / `status: "split_capacity_exceeded"`**: `escalate.to_id` / `escalate.message` を**そのまま** `send_message` に渡す（**verbatim 転送**。ja 側で言い換え・要約・翻訳をしない。日本語の文脈が要る場合も**本文の前に 1 行**の見出しまで。詳細規律は [`.dispatcher/references/spawn-flow.md`](references/spawn-flow.md) 3-1c）。文面は英語で届き、runtime 0.1.39 以降は**内容の異なる 5 種類**がある — renga 面 4 種（rect 候補ゼロ / タブ上限 `tab_limit_reached` / overflow 時の fleet ceiling 到達 / overflow しても新規タブが `MIN_PANE` 未満で助けにならない）と broker 面 1 種（`max_concurrent_workers` 到達）。ja 側に literal を持つと 1 種の旧形しか表現できないので持たない。該当ワーカー 1 件だけ派遣中止、監視ループは継続
  - **窓口報告には plan の診断フィールドを添える**: renga 面の `split_capacity_exceeded` では **`plan.layout`**（実測ペイン領域 / 左パネルが食う桁数と回収案内 / 新規タブ見積もり / tabs_seen）が診断オブジェクトになる。overflow 分岐まで到達した場合は **`plan.capacity`**（`max_concurrent_workers` / `active_workers` / `reserved_workers` / `free_worker_slots`）と **`plan.population`**（監査可能な worker census。`--peers-json` を渡したときだけ載る）も添える。broker 面は `plan.capacity` のみ（`layout` は renga 専用）
