...
- stall 検出 (Step 5) は通常 lookback `STALL_SECRETARY_LOOKBACK_MIN = 15` 分で評価するが、worker が `pr_opened` event を出して `pr_merged` が未記録の **PR-pending-merge sub-state** では `STALL_PR_MERGE_LOOKBACK_MIN = 60` 分に拡張する (Issue #304、session #12 の merge 承認待ち誤発火を抑制)。詳細は [`.dispatcher/references/worker-monitoring.md` Step 5 (b-2)](references/worker-monitoring.md)
