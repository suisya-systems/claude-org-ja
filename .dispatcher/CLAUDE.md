...
- **各サイクルの冒頭で `python3 ../tools/relay_scan.py --recipient secretary --audit` も 1 回叩く**（終端イベント relay の不実行検出、Refs #941。exit 0 = 対処不要 (`fresh` / `no_db`、および backlog が空なら `stale` / `never_scanned` もここ) / exit 10 = trace 欠落 **かつ** 未配送 backlog 実在なので窓口へ `RELAY_SCAN_STALE: ...` を送ってから通常どおり [Step 5.25](references/worker-monitoring.md) を実行する / exit 2 は窓口へエラー通知）。**relay の不実行は `event_deliveries` 台帳からは検出できない** — 配送対象が無かったサイクルも走らなかったサイクルも台帳に何も残さないため。`--list` が無条件に書く heartbeat だけが両者を弁別する。2026-07-30 〜 2026-08-19 の 20 日間、runbook が `python`（この環境には `python3` しか無い）と綴られていたため relay は毎サイクル `command not found` で空振りし、終端イベント 134 件が滞留したまま誰も気付けなかった。**上の spawn_gate と同じく、これがその沈黙に対する唯一の自動検出経路**である
