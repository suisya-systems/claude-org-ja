...
- **値がある**（`completion_reported_at != null`）→ 完了報告は既に secretary に届いている。**初回送信を発行せず**、本ゲートを acked 相当として通過し（「2. polling ループ」も回さない）retro を続行する。答えが手元にあるのに聞き直すと、窓口の受信箱を同じ 1 事実で埋めることになる（2026-08-08 に同一 task へ 4 回再送した実誤検知、Issue #869）
- **値が無い / record が無い / ファイルが読めない** → **記録の不在は「未着」の証拠にならない**（`WORKER_COMPLETION_NOTED` は secretary が best-effort・非 blocking で送るので取りこぼしうる）。従来どおり下の「1. 初回送信」から本ゲートを回す。ゲートの存在理由そのものが「dispatcher の受信キューに無い ≠ システム上に無い」だからである（下の「理由」節）
