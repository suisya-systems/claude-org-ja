...
## D-: 厳密な文字列等価ではなく「正文全体の包含」で照合する

**判断**: prompt から空白を全て除去したうえで、canonical 正文の **本文全体**
（`/loop [interval]` を取り除いた残り）が連続部分列として現れることを要求する。

**理由**:

1. 同じ正文が 2 つの包み方で届く。`CronCreate` の prompt にはコマンド名と interval を
   剥がした本文が入り、`ScheduleWakeup` の prompt には「`/loop` 入力をそのまま」＝
   `/loop 3m <本文>` が入る（各ツール定義の `prompt` 説明）。素の文字列等価を課すと
   どちらか一方が構造的に必ず deny される。
2. 包含にしても**短縮版・要約・言い換えは正文全体を含まないので通らない**。今回の事故は
   これで塞がる。「部分一致だと短縮版が通る」危険は *needle 側を短くした場合* の話で、
   needle は常に正文全体である。
