...
1. **`mcp__org-broker__poll_events` で直近のペイン lifecycle を drain** (タイムアウト付きで 1 回だけ):
   ...
   - **束縛が一致しなかったサイクル = cursor 破棄。黙って捨てない (MUST, fail loud)**。**適用条件は「捨てるべき cursor が実在したこと」**: cursor ファイルが**存在して中身が空でない**サイクルだけが本手順の対象で、**初回（ファイルが無い / 空）は破棄ではなく開始なので報告しない**（従来どおり `since` 省略で始めるだけ）。ファイルが在って JSON として読めないケースは「壊れた cursor を捨てた」なので対象に**含める**（`legacy_format`）:
     ...
     3. 窓口へ 1 行通知する（`mcp__org-broker__send_message(to_id="secretary", ...)`）:
        ...
        ```
     4. de-dup キーは **`(source=event_cursor, kind=stale_event_cursor, session_key)`** で、`session_key` は手順 2 で payload に載せた値を引く（**`(source, kind)` だけで引かない** — それでは 2 回目の restart が既存行に吸収されて黙る）。手順 1 で新しい束縛を書くので、正常なら次サイクルは一致して再送は起きない。`unbindable` が続く間は同じ `session_key=none` の行が既に在るので 1 回に抑えられる（毎サイクル同文を撃たない）。**`unbindable` が解けて別の `session_key` になった / 別の restart で別の `session_key` になった場合は、キーが変わるので必ず新しく 1 回報告される**
