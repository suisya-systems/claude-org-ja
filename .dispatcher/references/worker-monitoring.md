...
2. **`mcp__org-broker__check_messages` でワーカーからの自己報告を受信**:
   ...
   - **secretary→dispatcher 監視制御メッセージ (Issue #658)** — `WORKER_COMPLETION_NOTED` / `WORKER_REOPENED` を受信したら anomaly ではなく **lifecycle-control** として扱い、`.state/dispatcher/worker-idle-state.json` の該当 worker record の `completion_reported_at` を更新する (journal notify / de-dup スキーマには乗せない。**anomaly ledger でもない**):
     ...
     - `WORKER_REOPENED: worker-{task_id} (task_id={task_id}, reopened_at={ISO-8601 UTC})` → 該当 record の `completion_reported_at` を **`null` に clear** する。T6 (awaiting_review→in_progress、レビュー修正の追指示) で secretary が worker へ**直送**で再指示するとき送られる。再指示は secretary→worker 直送で dispatcher が経路上に居ないため、この明示 clear が無いと sticky skip でレビュー修正中の**本物の** silent dead-lock を永久に見逃す (解除契約、Issue #658 Blocker 対応)
     - どちらも secretary が **best-effort・非 blocking** で送る (secretary は dispatcher ack を待たない)。push 一次では channel 注入、フォールバック時は本 Step 2 の能動 `check_messages` で受ける。取りこぼしても次の完了受領 / 再指示で再送・再セットされる (record が無ければ Step 5.2 (b)(6) は `completion_reported_at` を欠損 = `null` 扱いで評価するため、取りこぼしは「監視が過剰に働く」安全側に倒れる)
...
5. **stall 検出 (STALL_SUSPECTED)** — 「stuck」と「Secretary 判断待ち idle」を補助シグナルで区別する独立チャネル:
   ...
   **idle streak の保持** (worker ごとに per-pane で永続化):
   ...
   - 各 worker のレコード形式 (例):
     ...
     ```
   - **`completion_reported_at` schema (Issue #658)**: `null | ISO-8601 UTC`。worker が完了報告を出し secretary が `WORKER_COMPLETION_NOTED` を送った時刻 (`received_at`) を保持する review-待ち印。既存 record で **field 欠損は `null` 扱い** (= 完了未報告) とし migration は不要。set / clear は Step 2 の監視制御メッセージ処理が行い、削除は下記更新規則 (4) の record 削除に従う (timeout による自然失効は持たない、(g) 参照)
...
5.2. **PANE_OUTPUT_WITHOUT_PEER_MSG 検出** — Step 5 / 5.1 の sibling、ワーカーがペイン上に応答出力したのに secretary 宛 `mcp__org-broker__send_message` を発行し忘れた **silent dead-lock** を検知する独立チャネル:
   ...
   **定数**: `PANE_OUTPUT_SETTLE_SEC = 60` (ペイン content が静止してから fire 候補とするまでの最小経過秒数、(g) で根拠説明)、`PANE_OUTPUT_IDLE_STREAK_MIN = 2` (= `idle_streak_cycles` の最低値。`/loop 3m` cadence で実効 ~6 分の完全静止を要求。(g) で根拠説明)。**look-back window は持たない** (worker→secretary 痕跡の比較は `last_content_change_ts` 起点で行い、(g) Codex round 1 Blocker 対応で 15 分窓は意図的に削除)。値変更時は本ファイルのこの行を直接書き換える。
   ...
   6. **かつ `completion_reported_at` が `null` である (Issue #658)**。record の `completion_reported_at` が非 `null` (= secretary が `WORKER_COMPLETION_NOTED` を送った = worker 完了報告済みで review 待ち) の worker は、追加の peer-msg を出さない idle が **正常** なので本検知の対象外。この gate は最優先で評価し、非 `null` なら (c) の SQL query を **発行せず** (d) の completion-review-skip 経路へ直行する (完了報告後に review 待ちで idle な worker を silent dead-lock と誤判定していた false positive の直接原因を断つ)。解除は T6 再指示の `WORKER_REOPENED` (→ `null` clear) / CLOSE_PANE・pane 消失 (→ record 削除) の lifecycle event のみ (Step 2 / 更新規則 (6))
   ...
   **task_id の SQL 文字列補間について** (Codex round 1 Minor): claude-org-ja の task_id は slug 規約 (`[a-z0-9-]+`、CLAUDE.local.md で task_id 値が決まる時点で人間 / runtime helper の制約で実質遵守、引用符 / SQL metachar は事実上含まれない) を満たすため、上記の string interpolation は安全。helper script 化 (sqlite3 `.parameter` 渡し or Python `tools/state_db/queries.py` 拡張) は将来課題で、現状は dispatcher Claude が SQLite one-liner を直接実行する。slug 以外の値が混入する経路 (例: 旧 worker_id 直書き) を将来導入する場合は、helper script 経由必須に切り替える。
   ...
   #### (d) 分岐 (completion-review-skip vs acked vs fire vs no-baseline)
   - **completion-review-skip** — (b)(6) の gate で `completion_reported_at != null` (= worker 完了報告済み・review 待ちの正常 idle、Issue #658): **fire しない**。(c) の SQL query は発行せず soft-note のみ追記して次サイクルへ:
     ```bash
   ...
   - **何が検出不能になるか / 許容根拠 (3 問への回答)**: 完了報告後・レビュー待ち中の worker が pane に追加出力しても PANE_OUTPUT は検出しない。state semantics 上 `review` は「worker 側の作業 in-flight なし」なので、この idle は正常であり許容できる。**ただし T6 再指示 (awaiting_review→in_progress) 後まで skip が残ると、レビュー修正中の本物の silent dead-lock を見逃す**。これを防ぐのが `WORKER_REOPENED` による clear 契約 (Blocker) で、再指示は secretary→worker 直送で dispatcher が経路上に居ないため、secretary が明示的に dispatcher へ `WORKER_REOPENED` を送って監視を再開させる。この clear が無ければ本 gate 導入は net で監視穴を作るため、clear 契約と一体で成立する
