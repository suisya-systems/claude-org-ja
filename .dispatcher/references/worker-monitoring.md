...
3. **`mcp__org-broker__list_panes` でペイン一覧を取得して突き合わせ**:
   ...
   **(3-a-3) 「在」と観測した履歴を控える** — 裏取り (ii) の突き合わせ先を `worker-idle-state.json` に持つ:
   ...
   | `bound_server_pid` / `bound_server_endpoint` | `null \| 数値` / `null \| 文字列` | 同上 (spawn 時の `server_info` の `server.pid` / `server.endpoint`) | `bound_pane_id` / `bound_peer_id` を発行した **backend session** の識別子。使用前に毎サイクル照合する ((3-a-5) の session provenance)。**`bound_pane_id` と必ず 3 つ揃えて持つ** — pane id の一意性保証は session に閉じており、daemon restart 後の同じ数値は別の生きたペインを指す (契約 T-§4.2-id)。**監視サイクルは spawn ターンの手元値を持たない**ので、record に書いていない面は照合しようがない |
   | `bound_cwd` | `null \| 文字列` | 同上 (spawn に渡した `{workers_dir}/{task_id}` の絶対パス) | (3-a-5) の identity 再照合で **org 束縛**を担う弁別子。予約名は別 org の並走タブに同名で実在するので `name` / `role` だけでは自 org のペインだと言えない (契約 T-§4.2-id の (O2)「bound to the expected organisation」) |
