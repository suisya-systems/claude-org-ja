...
- **同一タブ内 spawn で起動する理由**: pane 制御の scope が one tab wide だから（結論は契約 T-§4.2 の「Retained from ratified §4.2」で retained）。根拠は renga の版数ではなく **capability を確立できたか**で読む（各トークンは独立判定 = 契約 T-§cap の independence rule）:
  - `caller_scope`（Group A: `list_panes` / `focus_pane` / `inspect_pane` / `send_keys` / 各 `spawn_*` の `target`）と `caller_scope_close_identity`（Group B: `close_pane` / `set_pane_identity`）は**個別に判定するトークンで、一方から他方を導出しない**。確立できた側だけ対象が **caller のタブ**（フォーカス非依存）に解決される。組み合わせによらず scope は one tab wide なので、`new_tab` で別タブに置いた worker はディスパッチャーの `inspect_pane` / `close_pane` / `list_panes` から観測できなくなる
  ...
  - **確立できたトークン側の呼び出しでは解決タブが caller の所有タブなので、タブを切り替えても `pane_not_found` は解消しない**（タブ切替を復旧手順にしない）。確立できていない側は従来どおりフォーカス依存で、`caller_scope` だけの経路では `close_pane` / `set_pane_identity` が**ユーザーが見ているタブ**に解決されうる（契約 T-§4.2「Fail-safe consequence for Group B」。`close_pane` は不可逆なので相対セレクタを避け数値 id で当てる）。分岐手順は [`.claude/skills/org-delegate/references/renga-error-codes.md`](../../.claude/skills/org-delegate/references/renga-error-codes.md) の `pane_not_found` / `split_refused` 行、契約は [`docs/contracts/backend-interface-contract.md`](../../docs/contracts/backend-interface-contract.md) T-§4.2
