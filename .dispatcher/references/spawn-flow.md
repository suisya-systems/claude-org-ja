...
>
> **renga 面で `max_concurrent_workers` が無視されるのは caller タブ内に配置する既定経路のときだけ** (runtime 0.1.39 以降): `--overflow-to-new-tab` を armed にすると rect 上限が消える代わりに **fleet ceiling が live になり**、`--peers-json` が必須になる（欠落は `input_invalid` / exit 1 で拒否。overflow で置いたワーカーは自分のタブに居て `--panes-json` に二度と現れないため、census 無しでは上限が 0 のままタブを無制限に増やす）。**現行 ja はこのフラグを渡していない**（repo 全体で `--overflow-to-new-tab` / `--peers-json` の出現 0 件）ので、今日の運用では従来どおり無視される。正準条件は runtime CLI の `ceiling_applies`（`transport == "broker" or (transport == "renga" and --overflow-to-new-tab and --tab 未指定)`）。
...
1. `mcp__renga-peers__send_message(to_id=..., message=...)` を呼び、**helper が返した `escalate.to_id` / `escalate.message` をそのまま渡す**。本文は **runtime 生成物の verbatim 転送**であり、ディスパッチャーが自前の文面を組み立てる余地はない:
   ...
   - **日本語の文脈が要る場合は、本文の前に 1 行だけ** ja 側の見出しを足してよい（例: `SPLIT_CAPACITY_EXCEEDED（runtime 原文をそのまま転送します）`）。**前置 1 行までが許容範囲で、本文そのものの改変は不可**
