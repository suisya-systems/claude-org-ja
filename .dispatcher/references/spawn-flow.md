...
- **同一タブ内 spawn で起動する理由**: pane 制御の scope が one tab wide だから（結論は契約 T-§4.2 の「Retained from ratified §4.2」で retained）。根拠は renga の版数ではなく **capability を確立できたか**で読む（各トークンは独立判定 = 契約 T-§cap の independence rule）:
  - `caller_scope`（Group A: `list_panes` / `focus_pane` / `inspect_pane` / `send_keys` / 各 `spawn_*` の `target`）/ `caller_scope_close_identity`（Group B: `close_pane` / `set_pane_identity`）を確立した経路では、対象が **caller のタブ**（フォーカス非依存）に閉じる。`new_tab` で別タブに置いた worker はディスパッチャーの `inspect_pane` / `close_pane` / `list_panes` から観測できなくなる
