...
| kura | kura-data-aggregator-trial | https://github.com/aainc/kura-data-aggregator-trial | Salesforce 等の業務データを Postgres に集約し MCP 経由で参照させるデータ集約基盤 (Python + Docker + Terraform/GCP、Cloud Run job による日次 formatter) | コネクタ追加・改修、スキーマ / ビュー更新、formatter・MCP サービスのデプロイ整備、Issue 対応 | | develop |
