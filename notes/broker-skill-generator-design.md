...
- **pivot 入力（provenance）**: [`notes/broker-skill-gen-design-input.md`](./broker-skill-gen-design-input.md)
  ...
  **当該メモの「frontmatter = renga byte 不変」は §0.4 / §9.2 #9 で per-transport render へ訂正**（union は
  auth 迂回欠陥で却下、§9.2 #9 で per-transport が ratified。メモ側にも訂正注記あり）。
...
rollback 運用のトレードオフ（モデル 1 vs 2、§9.2 #9）は **2026-06-16 に人間が モデル 1 を ratify**（union は却下）。
**モデル 1（per-transport render + 再生成、auth クリーン）**では frontmatter は **per-entry 接頭辞リネーム**
...
> **role 必須引数**で broker tier は role により異なる（`user_common`/worker/curator=messaging 4 / dispatcher/secretary=ops tier。
...
> （`mcp__renga-peers__<tool>` → `mcp__org-broker__<tool>`）し、**ツール集合（= source 集合の像）は不変**に保つ。
> **ワイルドカード `mcp__renga-peers__*` だけは例外で `*` のまま broker へ写してはならない**（broker 固有/将来ツール
> まで広げるため。必ず ※5 の手順で renga source surface へ明示展開してから per-tool リネームする）。実装は
> `transport.fq_prefix` の per-entry 適用（`{{FQ}}` トークンと同じ機構）+ ※5 展開で、新しい SoT を作らない。
>
...
> per-transport・片方 union は不整合）。**人間判断（2026-06-16 ratified、§9.2 #9）= モデル 1（per-transport +
> 再生成、auth クリーン）を採用。モデル 2（union）は broker auth 迂回欠陥で却下**。なお**この緊張は skill
> frontmatter 固有ではなく、settings.local.json の per-transport 生成（promotion-plan §3）と §5.6「再生成なし
> 切戻し」の間に既に内在**しており、prose
...
drift CI（§7.2）が「broker 面の本文 + broker 面の frontmatter（== ※5 で正規化した source 像 = ワイルドカード展開 +
drop-filter 後。単純接頭辞置換ではない）」を assert し、さらに「renga 再生成が byte 安定（恒等）」を回帰固定する。
...
2. **frontmatter per-entry リネーム assert（§2.2 ※3/※4/※5）**: 各 skill 生成物の frontmatter `allowed-tools` が
   (i) committed の broker 面で **「source を ※5 で正規化した像」と byte 一致**すること。**期待値の作り方**:
   明示 per-tool エントリは server 接頭辞リネーム、**ワイルドカード `mcp__renga-peers__*` は renga source surface へ
   展開してから per-tool リネーム**、いずれも ※4 で broker 省略ツールを drop した結果（= **expanded/drop-filtered
   source image**）。**単純な接頭辞置換と比較してはならない**（ワイルドカード skill で `mcp__org-broker__*` を
   許してしまうため）。(ii) broker 省略ツールが drop され drift ログに記録、(iii) **renga 再生成が byte 安定**
   （接頭辞を renga へ戻すだけ＝恒等）、(iv) **生成 frontmatter に broker ワイルドカード（`mcp__org-broker__*`）が
   無く、broker 集合 ⊆ source 集合の像**であること。**role-tier への拡大が起きていない**（例: `send_message` のみの
   スキルに ops tier が付いていない）ことを明示 assert。
...
2c. **role assert（§2.2 ※2）**: role を要する全エントリ — `identity-anchor`（permissions.md、**`user_common`
   ブロックを含む**）と **ワイルドカードを持つ skill** — に `role` が存在し有効集合（**`user_common`** /
   worker / curator / dispatcher / secretary）であること。`user_common` は org-setup の user-common permissions
   ブロック（`allow_entries("user_common")` で broker 射影）に対応し、role 集合から漏らさない。明示 per-tool のみの
...
## 9. 推奨案サマリと open decisions（**全 9 件 ratified 2026-06-16**）
...
### 9.2 open decisions（**全 9 件 Ratified 2026-06-16**）

> **Ratified 2026-06-16**: 下表 **全 9 件を承認**（#1-#8 は本設計の推奨どおり一括承認、#9 は per-transport 訂正案を
> 採用＝下記）。確定値:
...
> - #9 = **per-transport render 採用**（broker 単一面 + per-entry 接頭辞リネーム + `ORG_TRANSPORT=renga` 再生成
>   切戻し）。**当初 ratify した transport-union は broker auth 迂回欠陥で却下**（broker 既定で renga 専用ツール
>   `focus_pane`/`new_tab` 等が認可されたまま＝auth モデル迂回。union の「再生成なし切戻し」利点も settings.local.json
>   が per-transport 生成のため成立しない）。§0.4 / §2.2 ※3-※5 / 本表 #9 行参照。
>
> **全 9 件 ratified により、実装フェーズ（Phase 3'）はこの確定設計を前提とする**。本表は以降「確定済み決定の記録」
> として読む（未決事項はない）。
...
| 9 | **skill frontmatter `allowed-tools` の auth モデル（broker auth 整合 vs env-only 切戻し）** | **Ratified 2026-06-16: per-transport render 採用**（broker 単一面 + per-entry 接頭辞リネーム + `ORG_TRANSPORT=renga` 再生成切戻し＝モデル 1）。**union は broker auth 迂回欠陥で却下**（broker 既定で renga 専用ツール `focus_pane`/`new_tab` 等が認可されたまま＝auth モデル迂回。union の「再生成なし切戻し」利点も settings.local.json が per-transport 生成のため成立しない）。skill frontmatter は settings.local.json の auth モードと一致 | 単一の静的 frontmatter では「broker 面で renga を出さない」と「再生成なし env-only 切戻し」を同時に満たせない（Claude Code は allowed-tools を実行時 transport で条件分岐不可）。**この緊張は settings.local.json の per-transport 生成（§3）と §5.6 に既に内在**。人間判断: auth 整合を優先しモデル 1（per-transport + 再生成）を採用。契約 §5.6「再生成なし」の語感は「env flip + org-setup 再生成（コード変更なし）」へ読み替える |
