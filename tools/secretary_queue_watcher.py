...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        offset = queue.stat().st_size
    except FileNotFoundError:
        # broker が後からファイルを作るケースを許容し、先頭から tail する
        offset = 0

    pending: list[float] = []  # 新規の owner 宛 enqueue の ts（enqueue 順）
    delivered = 0  # 新規の owner への delivered 件数
    ...
    while True:
        ...
        for line in chunk.splitlines():
            ...
            if not isinstance(rec, dict):
                continue
            ev = rec.get("event")
            if ev == "message_enqueued" and rec.get("to_id") == args.owner:
                ts = rec.get("ts")
