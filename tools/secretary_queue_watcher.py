...
- **broker run 境界でのリセット**: broker の in-memory queue は再起動をまたいで
  ...
  もう配達されえない残骸である。pre-scan は最後の `broker_started` 以降だけを数え、
  live 中に `broker_started` を見たら既存 backlog を 0 に戻す。これをしないと、
  過去 run の消失分が既存 backlog に紛れ込み、新規メッセージの配達を横取りして
  「配達済みなのに滞留」の誤発報になる。
...
def main(argv: list[str] | None = None) -> int:
    ...
    while True:
        ...
        for line in chunk.splitlines():
            ...
            ev = rec.get("event")
            if ev == "broker_started":
                # broker が再起動した: 起動前 backlog はもう配達されえない。
                # 本セッション中の pending は残す（再起動で消えたなら未配達のまま
                # 閾値超過で発報される = 真の消失として扱う）
                pre_backlog = 0
            elif ev == "message_enqueued" and rec.get("to_id") == args.owner:
                ts = rec.get("ts")
