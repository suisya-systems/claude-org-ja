...
- `block-dangerous-git.sh`: 素の `git push --force` / `-f` および protected branch (main / develop / release/* / production) への `git push --force-with-lease` をブロック。非保護 branch への `--force-with-lease` は許容（PR rebase / squash 後の安全な再 push 用、Issue #470）。加えて `git reset --hard` / `git branch -D` / `git clean -f` / `git checkout -- .` / `git restore --source` / `git tag -d` / `git update-ref -d` / `git reflog expire --all` 等の破壊的操作も引き続きブロック
