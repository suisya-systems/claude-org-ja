...
3. **DB に Status / Resumed を書く**（post-commit hook が `.state/org-state.md` の Status 行を `ACTIVE` に更新し `.state/journal.jsonl` も再生成する）:

   ```bash
   python -c "from datetime import datetime, timezone; from pathlib import Path; from tools.state_db import connect; from tools.state_db.writer import StateWriter; from tools.state_db.snapshotter import post_commit_regenerate; conn=connect('.state/state.db'); w=StateWriter(conn, claude_org_root=Path('.')); ts=datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%fZ'); w.begin(); w.update_session(status='ACTIVE', resumed_at=ts, updated_at=ts); w.commit(); post_commit_regenerate(w.conn, Path('.')); conn.close()"
   ```
