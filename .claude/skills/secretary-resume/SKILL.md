...
三値判定を dispatcher / handover 記載の各ワーカー名に適用し、**「在」（同タブ・一意）だけを現存として
数える**。他タブに同名のディスパッチャー / ワーカーが見えていても自組織の生存根拠にしない。
**「不在」は差分として、「unknown」は判定不能として区別して人間に報告する**（unknown を不在に数えない。
§1-2-b）。差分があっても勝手に再 spawn しないのは従来どおり。
