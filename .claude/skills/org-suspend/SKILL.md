...
1. **id 台帳を作る**。messaging とペイン制御は**別々の面**なので、id もそれぞれの面から取る
   ...
   - **突合（join）キーは `name` だけで、`role` は使わない**。`role` は非一意で（worker が複数居れば
     ...
     限る**。2.0 系の `list_peers` は**全タブ**を返す一方 `list_panes` は**単一タブ分しか返さない**
     （契約 §1.5 は本 amendment で非 supersede。どのタブかは上記のとおり capability 依存）ため、
     ここを誤ると別タブのピアのレコードに自タブの同名
...
2. Pane Name を Phase 1 の**台帳**で **`pane_id`** に解決する。`inspect_pane` はペイン制御面なので、
   ...
   - 台帳に `pane_id` が無い（＝ `list_panes` に居ない）ピアは 3 通りに分かれる。**いずれもスクレイプは
