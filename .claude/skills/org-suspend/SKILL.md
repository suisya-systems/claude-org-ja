...
- **gate は「capability 広告 backend の初回駆動」に対して 1 度だけ**であって、`/org-suspend` の実行ごとでは
  ...
  **ただし T-§ratification には第 3 の gate（production-activation）がある**: capability branch を
  ...
  - **参照（gate に入る前）**: events テーブルに過去の通過記録があるかを見る。あれば gate を発動しない
    ```bash
    python3 -c "from tools.state_db import connect; from tools.state_db.discover import resolve_state_db_path; \
    print(bool(connect(resolve_state_db_path()).execute(\"SELECT 1 FROM events WHERE kind='notify_sent' \
    AND payload_json LIKE '%capability_first_drive%' LIMIT 1\").fetchone()))"
    ```
    **DB のパスは `resolve_state_db_path()` で解決する**（`.state/state.db` 直書きにしない）。記録側の
    `tools/journal_append.py` が同じ解決を使うので、`STATE_DB_PATH` 等で別 DB に向いている環境では
    直書きだと**記録した DB と照会する DB がずれ**、通過記録が永久に見つからず gate が毎回発動する。
  ...
  - **記録（gate を通過した直後）**:
    ...
    ```
  - 以後の `/org-suspend`（[`.claude/skills/org-down/SKILL.md`](../org-down/SKILL.md) 経由の suspend を含む）は
    この記録に従う
