...
2. **`already_consumed`（live `.md` なし・`.consumed.md` あり）**: live handover が無い
   ...
   - `mcp__org-broker__list_panes` / `mcp__org-broker__list_peers` で **監視対象が live か**を確認する: active な
     ...
     **unknown のワーカーは監視対象・active のまま残す**（`.state/workers/worker-*.md` を
     `Status: pane_closed` にせず `WORKER_PANE_EXITED` も送らず
     `.state/dispatcher/worker-idle-state.json` の record も消さない）。再評価は再 arm した
     `/loop 3m` の通常サイクルが行う。
...
2. **caller pane id を確定する**（`focused: true` は「そのタブで今フォーカスされているペイン」で
   あって caller 自身ではない。人間がフォーカスを移した瞬間、別ペインを `dispatcher` に改名し、
   その id を `dispatcher_pane_id` として DB に焼き付けてしまう）: 環境変数 `RENGA_PANE_ID`
   （`echo "${RENGA_PANE_ID:-unset}"`）を起点に、`mcp__org-broker__list_panes` へ**同一 id のレコードが
   在ることを照合**する。ここで確定した id が Step 2 で書く `dispatcher_pane_id` であり、以後の
...
3. 照合できたレコードの name/role を確認:
   ...
   - **`caller-id 未確定`**（`RENGA_PANE_ID` 未設定 / `list_panes` に同一 id 無し / broker 等で同等の
     caller-id を入手できない）: 自己修復を実行せず、**`dispatcher_pane_id` も渡さない**まま
     （Step 2 の `update_session()` は未指定を「保持」と解釈するので既存値が温存され、これ自体が
     fail-closed になる）Step 1 へ進み、identity 未確認であることを Step 6 のブリーフィングで
     secretary に明示して続行判断を仰ぐ（`target="focused"` や裸 `name` へフォールバックしない）
4. `mcp__org-broker__list_peers` で `name == "dispatcher"` の `peer_id` を取得。
   **呼び出しの直前に
   ...
   かつ未承認なら列挙から `peer_id` を採らず破棄し、2 で確定した caller pane id で identity を
   確定して進む（停止しない・待ち時間 0 分）。
   **このとき Step 2 の DB 書き込みでは `dispatcher_peer_id` を渡さない**（`update_session()` は
   None / 未指定を「保持」と解釈する契約なので、**既存値がそのまま残る**）。placeholder や
   観測できなかった値を書くと、以後の routing が壊れた id を掴む。`dispatcher_pane_id` は
   2 で確定した caller pane id をそのまま書く（`caller-id 未確定` のときは渡さない）。Step 6 のブリーフィングでは
   ...
   **capability 形かつ承認済み（同 reference §2 の `first_drive` が `recorded`）のときも、自分の
   ...
   `name == "dispatcher"` に一致したレコードは**同タブに 1 件だけ在っても自分ではありえず**、
   他タブのみ / 同タブ性 unknown / `same_tab=True` が複数の各分岐と合わせて、**名前引きの結果が
   何であれ `peer_id` は未確定のまま**先へ進む。**したがって Step 2 の DB 書き込みでは
   `dispatcher_peer_id` を渡さない**（`update_session()` の「未指定 = 保持」で既存値を温存する。
   生きた同一ペインを引き継ぐ resume では保持が正しく、渡さないこと自体が fail-closed になる）。
   更新するのは 2 で確定した caller pane id（`dispatcher_pane_id`）だけである。Step 6 の
   ブリーフィングでは `peer=<未取得: 名前引きでは自 peer を同定できない>` と明示して secretary に伝える。
...
**Step 0 で `peer_id` が未確定だった場合（capability gate の縮退中、または承認済み路で名前引きから
自 peer を同定できなかった場合）は、`dispatcher_peer_id` を渡さない形で呼ぶ**。上の例をそのまま
使うと観測していない値を書き込むことになる:
...
**Step 0 が `caller-id 未確定` に落ちていた場合は、同じ理由で `dispatcher_pane_id` も渡さない**
（両方とも未確定で `update_session()` に渡す引数が残らないなら、この Step 2 自体を skip する）。
`focused` から採った id で埋めてはならない — 人間のフォーカス位置に依存した別ペインの id を、
以後の routing の宛先として恒久的に焼き付けることになる。

`update_session()` は未指定を「保持」と解釈する契約なので、既存値がそのまま残る（生きた同一ペインを
引き継ぐ resume ではこれが正しい。詳細は Step 0 手順 3 / 手順 4 と共有 reference §1-2-d）。
...
**capability 形かつ承認済み（同 reference §2 の `first_drive` が `recorded`）のときは、§1-2 の三値判定を
handover 記載の各ワーカー名に適用する。「在」は生存として扱う。「不在」だけでは `WORKER_PANE_EXITED` を
送らず、送信可否は §1-2-c の pane 面裏取りが取れたかに従う。「unknown」でも送らず、indeterminate として
secretary に報告する。**（判定条件・三値の定義・評価順の正本は §1-2。ここに重ねて書かない）
**unknown / indeterminate のワーカーは監視対象・active のまま残し**、Step 5 で再開する `/loop 3m` の
通常サイクルが再評価する。
...
1. **inflight 再生成（先に実行）**: Step 4 の `list_peers` / `list_panes` に
   ...
   [`.dispatcher/references/pane-close.md` 5-3](../../../.dispatcher/references/pane-close.md) の
   JSON。`trigger_task_id` は inflight を失った前 session の CLOSE_PANE 対象を指すフィールドで
   resume 側に値の出所が無いため書かない — Step 5.3 の完了受領 / timeout 判定はこの
   フィールドを読まない。`curate_result` は受領した `CURATE_*` を次サイクルへ持ち越す
   フィールドで、resume 時点では何も受領していないので**初期値 `null` を明示して書く** —
   Step 5.3 (a) が close できないまま inflight を保持するときだけ `"done"` / `"skipped"` /
   `"error"` に更新される）。
   ...
   **手書きの local(JST)時刻を `Z` で書かない**（再生成 started_at が未来時刻になると Step 5.3 の
   ...
   `list_peers` にのみ `curator` 名があり `mcp__org-broker__list_panes` の列挙に該当レコードが無い場合は
   **再生成せず** secretary に informational として報告する（数値 pane id を採れないうえ、
   messaging 面の名前一致は同タブの pane 生存の根拠にならない。Step 4 と同じ規律で、
   契約 T-§4.2 が pane 制御可能性の推論を禁じている）。
...
- Step 0 が `caller-id 未確定` に落ちていた場合は `pane=<未確定: caller pane id を同定できない>` と
  書き、`(state.db 更新済)` は付けない（前値保持のままであることを secretary に伝える）
