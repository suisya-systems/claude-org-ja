...
| exit | status | 窓口の対応 |
|---|---|---|
| `0` | `no_candidates` | 候補ゼロ。「いま着手可能な（依存解決済みの）候補はありません」と人間に伝える。`excluded_blocked` があれば「依存未解決で待ち」の件数を添えてよい。**ここで停止**。 |
...
1. [推奨] #531 Add retry to uploader（優先度 medium / 工数 S(推定) / 依存解決済み / 並列可(推定)）
   └ 優先度 medium・並列可（空き pane を埋められる）・工数 S(推定)・依存解決済み
2. #533 Refactor config loader（優先度 medium / 工数 S(推定) / 依存解決済み / 並列可(推定)）
...
- `candidates[]` を `rank` 昇順に番号付きで並べる。各行: `#<issue> <title>（優先度 <priority> / 工数 <effort>(推定) / 依存解決済み / 並列可(推定)）`。
- **推奨は 1 件だけ**。`recommendation.issue` に一致する候補の先頭に `[推奨]` を付け、直下に `└ <recommendation.reason>` を添える。
- **推定値には `(推定)` を必ず付す**（`effort_estimated` / `parallelizable_estimated` /
  `unblocked_by_recent_merge_estimated` が `true` の軸）。「機械が断定した」と人間が誤読して着手判断を機構へ
  明け渡すこと（設計 §4.4）を防ぐため。`parallelizable` が `false` なら「並列可」表記自体を出さない。
