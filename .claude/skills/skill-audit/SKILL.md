...
# 公開の skill-candidates.md（フォーマット定義のみ・エントリ常に空）と
# machine-local な skill-candidates.local.md（実エントリ。Issue #755）の
# 2 ファイルを CANDIDATE_ENTRY_PATHS と同順で読む。存在するファイルだけを
# 位置パラメータ（set -- / "$@"）に積む: これは bash・zsh・sh いずれでも
# 正しく複数引数へ展開される（未クォートの `$files` は zsh が単語分割せず
# awk に 1 引数として渡してしまい、`2>/dev/null` で fatal を握り潰して
# カウントが空になる — 実ハーネスの shell は zsh のため必須）。ファイルが
# 1 つも無ければ zero-arg awk が stdin でハングするのを避けて 0 を返す。
...
> `~~~` で開閉するブロック内は除外）を **2 ファイル**
> [`knowledge/skill-candidates.md`](../../../knowledge/skill-candidates.md)（公開・
> フォーマット定義のみ）+ `knowledge/skill-candidates.local.md`（machine-local な
> 実エントリ。Issue #755・gitignore 済み）にわたり合算、work-skill は
> `find .claude/skills -maxdepth 2 -name SKILL.md | grep -v '/org-'`）は、
> オンデマンド curator の起動判定
> [`tools/check_curate_threshold.py`](../../../tools/check_curate_threshold.py) と
> **完全一致**させること。読む 2 ファイルとその順序は同ファイルの
> `CANDIDATE_ENTRY_PATHS`（Python SoT）と一致させる。pending カウントの
> セマンティクスはさらに公開ファイル冒頭の運用ルール記載と合わせた
> **3 者同期**（本 Step 1 / check_curate_threshold.py / skill-candidates.md 冒頭）。
> いずれかを変更する場合は 3 箇所同時に更新する（`tools/test_check_curate_threshold.py`
> の parity テストが drift を検出する — awk が読むファイル一覧が
> `CANDIDATE_ENTRY_PATHS` と一致することも検査する）。

> **`deferred` は数えない（Issue #753）**: 上の awk は `- **status**: pending` の行形式
> **完全一致**でカウントするため、`deferred`（人間が提示を受けて見送った候補）は自動的に
> 除外される。見送り済み候補を `pending` のまま残すと worker クローズのたびに `cand_count >= 5`
> を再発火させ curator を無駄起動するため、**バッチ問い合わせで人間が見送った候補は
> `pending` → `deferred` に更新**する（更新自体は Set E §1.4 の owner 定義に従いワーカー委譲経由）。
> **`deferred` を `pending` に戻さない・再問い合わせしない**（本 Step 1 の閾値にも再登場させない）。
> 人間が後日あらためて検討したいときのみ新しい日付で別エントリを起こす。
