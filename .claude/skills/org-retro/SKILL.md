...
キュレーター側から上がってきたプロジェクト固有ノートの提案（`org-curate` Step 3.5）も同じ扱いで、
受け取った窓口が provenance と privacy review を付して `registry/projects/<slug>/notes/` へ適用する。
キュレーターが `registry/` 配下に下書きを置くことは、下書きであっても許されない。
