...
> **輸送層 両系（`ORG_TRANSPORT`: 既定 `renga` / opt-in `broker`）**: 本テンプレを送る `send_message` は **既定 `renga`**（`ORG_TRANSPORT` 無設定）で `mcp__renga-peers__send_message`。`ORG_TRANSPORT=broker`（opt-in・切戻し可）では完全修飾名が **`mcp__renga-peers__*` → `mcp__org-broker__*`** に機械置換され（引数形は同一）、worker 側の受信は in-band push ではなく **pane-local ナッジ + `mcp__org-broker__check_messages` で pull**、完了 / 進捗報告の宛先指定（`to_id="{report_target}"`）も同型。詳細は [`docs/contracts/backend-interface-contract.md`](../../../../docs/contracts/backend-interface-contract.md) Surface 8（批准待ち）と [`.claude/skills/org-delegate/references/renga-error-codes.md`](renga-error-codes.md) の broker 節を参照。既定 renga の指示文は不変（broker は加算）。

## ultracode タスクでの worker 着手規約（Issue #554）

worker brief（`CLAUDE.local.md`）に ultracode 使用許可があるタスクでは、ultracode は **dispatcher がペインに `send_keys` で打鍵する `ultracode` kickoff（user turn）**でのみ武装される（この指示本文＝`send_message` / `check_messages` 経由の peer/channel メッセージ内に `ultracode` と書いても武装しない＝実走確定）。したがって worker 側の規約:
...
- 武装の詳細・dispatcher 手順の SoT は [`.dispatcher/references/spawn-flow.md`](../../../../.dispatcher/references/spawn-flow.md) の **3-5a**。許可文言を本テンプレに転記する必要はない（SoT は `CLAUDE.local.md` の implementation 指示）。
...
- **full**（新機能実装 / 修正 / リファクタ / テスト追加 / hook・skill・設定編集など、コードまたは挙動の変更を伴うもの）
  ...
  - **正規実行形**（Windows / worktree 実測知見: `knowledge/curated/codex.md`）:
    - `codex exec --skip-git-repo-check "<prompt>" > tmp/codex-review-<task-id>-round<N>.log 2>&1 < /dev/null &` — stdin を `< /dev/null` で明示クローズし、ラウンド別ログファイルに直書きする。実行中の codex exec を `| tail` でパイプしない（バッファリングで出力が空に見える）。ラウンドごとにログファイル名を変える（同一ファイルへ 2 プロセスが `>` で書くと混線する）
    - 完了検知はログ末尾の `tokens used` マーカーではなく**プロセス終了**で待つ（例: `until ! tasklist | grep -qi codex; do sleep 10; done`。マーカーは出力タイミングが遅れる / 途中の echo に紛れることがある）
    - Windows の PowerShell 制約言語モードでは codex のプローブが `rejected: blocked by policy` で多発するが、codex は別手段にフォールバックして完走する。**block 多発 = レビュー失敗と誤判定しない**
    - codex の判定サマリ（Blocker なし等）だけ読んで done にしない。終了後のログ末尾（`tail -c 8000` 等。全文 Read は 256KB 上限超過しうる）で**残留リスク自己申告**（policy block で未実行の検証）を確認し、未実行分は worker 側で手動再実行してから「codex clean」と報告する
...
> 成果物（{ARTIFACT_NAME}）はファイルに書き出さず、本文を 8000 字程度のチャンクに分割して renga-peers `mcp__renga-peers__send_message(to_id="secretary")` で順送信してください。各チャンク冒頭に `[CHUNK n/N]` ヘッダを付け、最後に `[CHUNK_END]` を送信。窓口側で結合し `{worker_dir}/{ARTIFACT_NAME}` として書き出します。Edit/Write を試みてはいけません（deny される）。
