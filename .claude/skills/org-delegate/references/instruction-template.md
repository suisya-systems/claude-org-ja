...
- **full**（新機能実装 / 修正 / リファクタ / テスト追加 / hook・skill・設定編集など、コードまたは挙動の変更を伴うもの）
  - commit 完了後に `codex` CLI が**インストールされていれば** `codex exec --skip-git-repo-check` 直打ちで Codex セルフレビューを実行（任意。`command -v codex` で確認）
  - codex 未導入環境ではセルフレビューを skip し、commit 後そのまま完了報告に進む（以下のラウンド規律は適用されない）
