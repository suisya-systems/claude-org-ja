...
- `{worker_dir}` 外の任意パス（親リポジトリの root を含む）

いずれも `read-only file system` で失敗する。

### 書き込みを拒否されたときの対処
**出力先を `$TMPDIR` 配下へ変えて再試行する**。これが正しい対処であり、サンドボックスの解除ではない。`read-only file system` は「その場所に書けない」という意味であって、「サンドボックスが邪魔をしているので外すべき」という意味ではない。
...
この規範はサンドボックス由来に見える失敗にも適用する。**書き込みや外部コマンドが失敗したとき、サンドボックス設定を真因と断じる前に、出力先を `$TMPDIR` 配下へ変えて再試行する反証を先に実施すること**（2026-08-27 に、この反証を先に行わずに真因を読み取り拒否と断じ、`dangerouslyDisableSandbox` の反復要求でセッションロックアウトに至った事故がある。詳細は上記「書き込み可能範囲と、書き込みを拒否されたときの対処」節）。
