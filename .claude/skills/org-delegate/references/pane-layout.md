...
窓口 (`secretary`) のみが立ち上がり、ワーカーは別タブに積んでいく方針。
...
Tab 2: worker-<task-id-1>    Tab 3: worker-<task-id-2>    ...
┌──────────────────┐         ┌──────────────────┐
│                  │         │                  │
...
| 各ワーカー | 新規タブで派生 | `ccmux new-tab --role worker --id worker-{task_id} --label {task_id} --command "cd {workers_dir}/{task_id} && claude ..."` (org-delegate Step 3) |
