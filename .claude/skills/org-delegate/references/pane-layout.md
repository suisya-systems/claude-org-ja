...
>
> **例外 — renga 面でも overflow 時は `max_concurrent_workers` が live になる**（runtime 0.1.39 以降）: `--overflow-to-new-tab` を armed にすると rect 上限が消える代わりに fleet ceiling が効き、`--peers-json` が必須になる（欠落は `input_invalid` / exit 1）。**現行 ja はこのフラグを渡していない**（repo 全体で出現 0 件）ので、今日の renga 運用では下記 rect 記述がそのまま律速する。
...
- CLI (運用上の標準呼び出し): `claude-org-runtime dispatcher delegate-plan --task-json ... --panes-json ... --state-dir ... --transport {broker|renga} [--max-concurrent-workers N] [--template-repo ...] [--locale-json ...]`。dispatcher は `ORG_TRANSPORT` を解決して `--transport` を明示で渡す（runtime は panes snapshot から transport を推定しない契約、runtime 0.1.31 / #104）。broker 面では `--max-concurrent-workers N`（既定 8。`unlimited` は opt-in）で worker capacity を gate する（renga 面は rect ベース balanced split が律速するため `--max-concurrent-workers` は効かない — ただし `--overflow-to-new-tab` armed 時のみ fleet ceiling として live になる。上記輸送層注記の例外を参照）。dispatcher 側の手順は `.dispatcher/CLAUDE.md` の delegate-plan helper 節を一次参照
