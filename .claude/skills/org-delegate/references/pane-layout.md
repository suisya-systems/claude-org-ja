...
- **raw キー送信**: `ccmux send --text` CLI 併用（開発チャネル Enter / permission mode 切替 Shift+Tab 等）。upstream happy-ryo/ccmux#118 で `send_keys` MCP が設計中。追加されたら後続 Issue で MCP に切替
