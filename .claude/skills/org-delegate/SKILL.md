...
- **worker へ ack を返す（dead-lock 防止のため常に最初に返すが、文面は次項のサマリ確認で分岐）**: full モードでは直後の「人間向け理解サマリ」確認の結果で ack 文面を選ぶ — サマリが揃っていれば [`.claude/skills/org-delegate/references/ack-template.md`](references/ack-template.md) の標準「完了報告 ack」（この後 user 承認に進む文面）、欠落していれば『サマリ補完依頼』ack（user 承認には進まず補完を待つ旨）を返す。minimal は標準「完了報告 ack」で進む
