...
ccmux-peers の `send_message` でフォアマン（pane name=`foreman`）に以下を送信する:
...
`$target` が空（候補セットが空）の場合、フォアマン Claude は **`spawn_pane` を発行せず**、代わりに ccmux-peers で窓口 (`secretary`) に escalate メッセージを送信する:

1. `mcp__ccmux-peers__send_message(to_id="secretary", message=...)` を呼び、本文を以下にする:
   ```
...
pane は live でも Claude がまだ起動中の場合があるため二重確認。`mcp__ccmux-peers__list_peers` を呼び、`worker-{task_id}` が peer 一覧に現れるまで短い間隔（例: 2 秒）でリトライする（最大 30 秒程度）。タイムアウトした場合は `list_panes` でペイン状態を再確認し、必要なら窓口に escalate する。

### 3-5. `mcp__ccmux-peers__send_message` でワーカーに指示を送信
...
(b) `mcp__ccmux-peers__inspect_pane(target="worker-{task_id}", lines=5, include_cursor=true, format="grid")` の `structuredContent.lines` からステータスバーを読み、「auto mode on」表示を確認する（または `mcp__ccmux-peers__send_message(to_id="worker-{task_id}", ...)` でワーカーに問い合わせる）— 最大 5 回リトライ
...
1. `.state/workers/worker-{task_id}.md` を作成（ccmux-peers では pane name `worker-{task_id}` が安定識別子。旧 peer-id は使わない）:
   ```markdown
...
5. 窓口 (`secretary`) に ccmux-peers で派遣完了を報告:
   ```
...
mcp__ccmux-peers__send_message(
  ...
  message="窓口です。{task_id} の作業をお願いしています。完了・進捗・ブロック、全ての報告は `to_id=\"secretary\"` で ccmux-peers 送信してください。"
)
```
...
ワーカーから ccmux-peers でメッセージを受け取ったら:
