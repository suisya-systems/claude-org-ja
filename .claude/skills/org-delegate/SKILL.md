...
> **state-db cutover (M2.1, Issue #272)**: 構造化セクション
...
> `.state/org-state.md` と `.state/journal.jsonl` を自動再生成するので、本 skill では
> markdown を直接編集する手順を残さない。`tools/journal_append.py` (および `journal_append.sh`) は
> M2 で DB ルーティング済みなので、そちらは引き続き使う。
...
2. **DB 経由で run と Active Work Items を登録する**（`.state/org-state.md` 直接編集は禁止。post-commit hook が再生成）:

   ```bash
   python -c "from pathlib import Path; from tools.state_db import connect; from tools.state_db.writer import StateWriter; from tools.state_db.snapshotter import post_commit_regenerate; conn=connect('.state/state.db'); w=StateWriter(conn, claude_org_root=Path('.')); w.begin(); w.register_worker_dir(abs_path='<worker_dir_abs>', layout='<flat|worktree|ephemeral>', is_worktree=<True|False>); w.upsert_run(task_id='<task_id>', project_slug='<slug>', pattern='<A|B|C>', title='<title>', status='in_use', worker_dir_abs_path='<worker_dir_abs>'); w.update_session(objective='<人間の依頼>'); w.commit(); post_commit_regenerate(w.conn, Path('.')); conn.close()"
   ```

   - free-form な「Current Objective の補足説明」は markdown 直接 append で OK（passthrough で保持）。`update_session(objective=...)` は構造化 header フィールドとして上書きされる
   - `pattern` は A / B / C のいずれか
   - 一行版は cross-shell で動く。Python from-script で書く場合は `with w.transaction():` を使う方が安全（rollback セマンティクスが効く）
...
2b. 人間が承認した場合（「OK」「確認した」「問題ない」等）:
   ...
   - **DB 経由で Worker Directory Registry を更新する**（markdown 直接編集禁止）:
     ```bash
     python -c "from pathlib import Path; from tools.state_db import connect; from tools.state_db.writer import StateWriter; from tools.state_db.snapshotter import post_commit_regenerate; conn=connect('.state/state.db'); w=StateWriter(conn, claude_org_root=Path('.')); w.begin(); w.update_run_status('<task_id>', 'completed'); w.commit(); post_commit_regenerate(w.conn, Path('.')); conn.close()"
     ```
     - パターン A: lifecycle='active' のまま、run.status='completed' で snapshotter が available 相当の表示にする
     - パターン B / C: 物理 dir は別途処理（worktree remove / dir 保持）。レジストリエントリ削除は同コマンドで `w.remove_worker_dir('<abs>')` を `w.commit()` の前に追加
   - JSON スナップショットを再生成する: `py -3 dashboard/org_state_converter.py`
