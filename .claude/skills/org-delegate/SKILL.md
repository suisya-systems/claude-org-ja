...
- 開発チャネルの確認プロンプトが表示されるので、`ccmux send --name worker-{task_id} --enter ""` で Enter を送信する（raw キー入力は ccmux リリース前のため CLI 併用。upstream happy-ryo/ccmux#118 / ccmux PR #122 で `send_keys` MCP の API 確定済み。**merge + リリース後**の置換形: `mcp__ccmux-peers__send_keys(target="worker-{task_id}", enter=true)` — 切替は #30 cleanup で一括）
...
(a) `ccmux send --name worker-{task_id} $'\x1b[Z'` で Shift+Tab を送信する（`--enter` を付けない）
    - **upstream merge + リリース後の置換形**: `mcp__ccmux-peers__send_keys(target="worker-{task_id}", keys=["Shift+Tab"])`（ccmux PR #122、API 確定済み、切替は #30 cleanup で一括）
...
(c) モード切替完了を確認してから、Plan を承認する（`ccmux send --name worker-{task_id} --enter "yes"`）
    - **upstream merge + リリース後の置換形**: `mcp__ccmux-peers__send_keys(target="worker-{task_id}", text="yes", enter=true)`
...
1. **先にモード切替**: ワーカーペインに Shift+Tab を送信する
   ...
   ```
   **upstream merge + リリース後の置換形**: `mcp__ccmux-peers__send_keys(target="worker-{task_id}", keys=["Shift+Tab"])`（ccmux PR #122、切替は #30 cleanup で一括）
