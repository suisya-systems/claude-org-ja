...
1. `set_summary` で自分のサマリーを設定する: 「Secretary: 窓口」
   - ワーカーが `list_peers` で窓口を発見するために必須
2. `registry/org-config.md` の `workers_dir` を読み、ワーカーディレクトリの存在を確認する。
...
### フォアマン
```
claude --dangerously-load-development-channels server:claude-peers --permission-mode bypassPermissions --model opus
```
...
1. `ccmux split` で窓口ペインを上下分割し、下半分にフォアマン用の Claude を起動する:
   ...
   ccmux split \
     ...
     --role foreman \
     --id foreman \
     --command "cd .foreman && claude --dangerously-load-development-channels server:claude-peers --permission-mode bypassPermissions --model opus"
   ```
