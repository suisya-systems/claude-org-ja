...
>
> **state DB 前提 (Issue #267 / M1)**: `.state/state.db` は markdown が SoT である間
> 冪等に rebuild 可能な derived view。M2 で write 切替するまでは markdown が canonical。
> 読み手は DB → markdown fallback の順で参照する。
...
1. `.state/state.db` が存在するか確認する
   - 存在 + markdown より新しい（または同等）→ DB を read primary として使う:
     ```bash
   ...
   - 存在するが markdown より古い（stale）→ 警告を表示し、Secretary に rebuild を促す:
...
5. ディスパッチャーのピアIDと renga ペイン名（`dispatcher`）を記録する（org-state.md の Dispatcher セクション）
