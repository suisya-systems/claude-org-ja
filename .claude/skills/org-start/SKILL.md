...
> **Issue #410 / Stage B**: Step 0 (set_summary / MCP 疎通 / identity 検証 / workers_dir 確認の 4 サブステップすべて) が完了した時点で dispatcher の `spawn_claude_pane` を発火し、Claude 起動待ち (〜30〜60s) と並列に Block B (前回状態の DB 読み込み) / Block C (ダッシュボード server 起動) を進める。serial 実行時の wall-clock 〜3 分を ~35s まで短縮する目的。
>
