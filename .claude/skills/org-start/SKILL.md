...
> **Issue #410 / Stage B**: Step 0 (set_summary / MCP 疎通 / identity 検証 / workers_dir 確認の 4 サブステップすべて) が完了した時点で dispatcher / curator の `spawn_claude_pane` を発火し、Claude 起動待ち (〜30〜60s) と並列に Block B (旧 Step 1) の状態復元 / Block C (旧 Step 1.5) のダッシュボード起動を進める。serial 実行時の wall-clock 〜3 分を ~35s まで短縮する目的。
>
...
4. **Block B の DB 初期化完了を待つ** — 並列実行の合流点。Block B-1 で `.state/state.db` 不在 → `importer --rebuild` が走った場合、schema 構築完了までは `StateWriter.update_session()` が失敗するので、Block D-4 の DB write 前に Block B が完了している必要がある。Block B が SUSPENDED ブリーフィング待ちで未完了なら、ブリーフィング完了 → DB schema 健全性確認 → ここに進む
5. **DB transaction を 1 回にまとめて dispatcher / curator 両方の identity を記録する** (`StateWriter.transaction()` 経由、markdown 直接編集禁止。post-commit hook が `.state/org-state.md` を再生成):
   ```bash
   python -c "
