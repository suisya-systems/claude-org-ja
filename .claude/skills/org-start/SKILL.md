...
> **Issue #410 / Stage B**: Step 0 の identity 検証が完了した時点で dispatcher / curator の `spawn_claude_pane` を発火し、Claude 起動待ち (〜30〜60s) と並列に Step 1 の状態復元 / Step 1.5 のダッシュボード起動を進める。serial 実行時の wall-clock 〜3 分を ~35s まで短縮する目的。
>
...
### Block A: ディスパッチャー / キュレーターペイン spawn (旧 Step 2 / Step 3 の発火部)
...
### Block C: ダッシュボードサーバー起動 (旧 Step 1.5)
...
### Block D: 両ペインの合流 (旧 Step 2 / Step 3 の後半をまとめて並列化)
...
4. **DB transaction を 1 回にまとめて dispatcher / curator 両方の identity を記録する** (`StateWriter.transaction()` 経由、markdown 直接編集禁止。post-commit hook が `.state/org-state.md` を再生成):
   ```bash
   python -c "
   ...
   "
   ```
5. JSON スナップショットを 1 回だけ再生成する（dashboard 用、state-db cutover とは別経路。両 identity をまとめて反映できるので 2 回呼ぶ必要はない）:
...
- **curator spawn 失敗 / dispatcher spawn 成功** — dispatcher は残してユーザーに報告する。組織機能の中核（ワーカー派遣・状態書き込み）は dispatcher 単独で維持されるため、**ユーザーには「dispatcher のみ起動済み。curator を再 spawn するか、curator 無しで暫定継続するか」を選択肢として提示**する。Block B / C / D の dispatcher 関連手順は curator 失敗とは独立に完走させる（DB write も dispatcher 分のみ実行）
- **両 spawn 成功・boot 中に片方が peer 登録されない** — Block D-2 の poll で片方が timeout する。該当ペインに Enter を再送 → 再 poll。3 回 retry してダメなら該当ペインを `close_pane` で破棄してユーザー報告
...
### Stage A / Stage B の wall-clock 効果 (Issue #410)

| stage | 変更内容 | wall-clock |
|---|---|---|
| 改修前 | Step 1 → Step 1.5 → Step 2 (spawn+Enter+poll+greet+DB+snapshot) → Step 3 (同様) を serial | 〜180s |
| Stage A 後 | Step 2 / Step 3 を 1 つの並列 block にまとめ、両 spawn / Enter / poll / greet / DB write / snapshot を一括化 | 〜90s |
| Stage A+B 後 | 上に加え、Step 0 識別子検証直後に spawn 発火、Block B (state 復元) / Block C (dashboard 起動) と並列に Claude の boot wait を overlap | 〜35s |
