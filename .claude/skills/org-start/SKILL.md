...
全インスタンス共通:
- `--dangerously-load-development-channels server:ccmux-peers`: ccmux-peers のチャネル通知を有効化（ピア通信に必須）
...
2. 開発チャネルの確認プロンプトが表示されるので、`mcp__ccmux-peers__send_keys` で Enter を送信する:
   ```
   ...
   - エラー時は `[<code>] <msg>` 形式のテキストが返る。分岐は `.claude/skills/org-delegate/references/ccmux-error-codes.md` 参照
