...
ccmux 0.14.0+ は `mcp__ccmux-peers__spawn_pane` / `new_tab` 経由で起動した **bare** `claude`（および `claude <args>`）を自動的に Alt+P 相当へ upgrade し、`server:ccmux-peers` チャネルを注入する。ただし auto-upgrade が発動するのは先頭トークンが `claude` であるときのみで、`cd X && claude ...` のように **cwd 変更プレフィックスが付くと auto-upgrade が効かず、channel push が届かない**（#46 のリグレッション）。長寿命ロール（Foreman / Curator）とワーカーはいずれも `cd` 前置きが必須なので、起動コマンドに `--dangerously-load-development-channels server:ccmux-peers` を **必須フラグ** として明示する。

### フォアマン
```
claude --dangerously-load-development-channels server:ccmux-peers --permission-mode bypassPermissions --model sonnet
```
...
### キュレーター
```
claude --dangerously-load-development-channels server:ccmux-peers --permission-mode {default_permission_mode} --model opus
```

### ワーカー（org-delegate の Step 3 で使用）

**モデルは必ず `--model opus` を指定する（sonnet 禁止）。**
...
```
claude --dangerously-load-development-channels server:ccmux-peers --permission-mode plan --model opus
```
...
1. `mcp__ccmux-peers__spawn_pane` で窓口ペインを上下分割し、下半分にフォアマン用の Claude を起動する:
   ...
   mcp__ccmux-peers__spawn_pane(
     ...
     role="foreman",
     name="foreman",
     command="cd .foreman && claude --dangerously-load-development-channels server:ccmux-peers --permission-mode bypassPermissions --model sonnet"
   )
   ```
   ...
   - `command` は新しいペインでシェルプロンプトが立ち上がった直後に注入される。`cd .foreman &&` を前置きする都合で ccmux auto-upgrade が発動しないため、`--dangerously-load-development-channels server:ccmux-peers` を明示する必要がある（#46 を revert）。このフラグが無いと `send_message` による channel push が届かず、フォアマンが窓口の指示を一切受け取れない
...
2. `--dangerously-load-development-channels` を付けると Claude Code 初回起動時に「Load development channel? (Y/n)」確認プロンプトが表示される。`mcp__ccmux-peers__send_keys` で Enter を送信して承認する:
   ```
   mcp__ccmux-peers__send_keys(target="foreman", enter=true)
   ```
   - Enter は CR (0x0D) として PTY に書き込まれる（ccmux 側の `append_enter` と byte-identical）
...
1. `mcp__ccmux-peers__spawn_pane` でフォアマンペインの右半分をキュレーター用に立ち上げる:
   ...
   - `cd .curator &&` 前置きで auto-upgrade が発動しないため、`--dangerously-load-development-channels server:ccmux-peers` を明示する必要がある（#46 revert）。フラグ無しでは channel push が届かず、キュレーターが /loop /org-curate 起動指示を受信できない
