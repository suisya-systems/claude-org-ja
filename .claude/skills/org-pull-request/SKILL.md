...
- **merge 承認提示でも人間向け理解サマリを再掲する（検証深度 `full` 限定）**: CI green → ユーザーに merge 承認を仰ぐ際、worker 完了報告の「人間向け理解サマリ」（(1) 最重要の変更点 N 個、(2) 要確認ファイル / hunk、(3) 設計判断と理由）を再掲し、ユーザーが diff を開かずに最終 merge 判断を下せるようにする。**再掲元は [`.claude/skills/org-delegate/SKILL.md`](../org-delegate/SKILL.md) Step 5 (2a) で `.state/workers/worker-{task_id}.md` Progress Log に `Human Understanding Summary:` 見出しで永続化済みのサマリ**を読む（複数回完了している場合は最新の見出しを使う。`/clear` / resume・CI 監視後も手元の元メッセージ本文に依存しない）。永続コピーが見つからない場合（本フォーマット導入前から in-flight の PR や、サマリ永続化前に PR 作成済みの run 等）は PR 本文 / worker 完了報告メッセージ / 差分確認に fallback し、可能なら同等の 3 点を窓口が再構成して提示する。完了報告受領時の承認提示（Step 5 (2a)）と同じサマリで、full モード完了報告スキーマの SoT は [`.claude/skills/org-delegate/references/worker-claude-template.md`](../org-delegate/references/worker-claude-template.md)（[`docs/contracts/role-contract.md`](../../../docs/contracts/role-contract.md) が指定）。minimal タスクには付かない
