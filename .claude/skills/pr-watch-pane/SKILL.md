...
> best-effort（broker send CLI 不在 / 解決先 transport が未構成の pane では
...
> **輸送層（transport）両系**: 本文のツール参照は `mcp__org-broker__*`（`org-broker`）で書いてあり、
> active transport が既定面ならそのまま従えばよい。輸送層で手順が変わるのは **受信モデル /
> spawn 儀式 / エラー分岐** の 3 点だけで、opt-in 側の輸送層は削除せず切戻しの安全装置として
> 常時有効。両系の読み替え規則・契約の批准経緯・broker 固有エラーコードの一覧は
> [`.claude/skills/pr-watch-pane/references/transport-duality.md`](references/transport-duality.md)
> を参照（上記 3 分岐に入るまで読む必要はない）。

> **本 skill の spawn 対象は「汎用 CLI ペイン」**: 起動するのは `tools/pr-watch.sh`
> という shell コマンドであり、Claude セッション（`spawn_claude_pane`）ではなく
> **汎用 `mcp__org-broker__spawn_pane`** を使う。したがって輸送層の **spawn 儀式（broker:
...
> （[`.claude/skills/pr-watch-pane/references/transport-duality.md`](references/transport-duality.md)
> 末尾の「attention watcher は ... 2 段承認の対象外」注記と同義）。両系で
> 切り替わるのは **ツール完全修飾名だけ**:
>
...
- `[name_in_use]` / `[name_taken]` → **live pane と stale 登録簿を切り分ける**（前 watcher の
  ...
  3. **live pane が無い**（`list_panes` に出ない）→ self-close 済みの **stale 登録簿 binding**。
     ...
     - **(1) いま Group B を駆動している backend が `close_pane` / `set_pane_identity` を自身の
       ...
       （契約 §8.1 / §8.10）。**判定は「いま Group B を撃つのに使っている MCP ツールの
       完全修飾名が `mcp__org-broker__*` である」という積極的な証拠で行う**。
       **`DEFAULT_TRANSPORT` から推定してはならない** — 判定規則と根拠の SoT は下記 Step 5 の (b)。
       確定できない場合は不成立として扱う
