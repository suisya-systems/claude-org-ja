...
task_ids: [<関連タスク ID>, ...]          # post_retro は通常 1 件、curation は複数
...
候補止まり層を明示的に作ることで、Codex 指摘の「skill 検索面ノイズ」を予防する。
...
- **org-curate（curation）**:
  ...
  - `candidate_queue` → raw/ は既存、curated/ には統合しない（次回待ち）
  - `curated_only` → 通常の Step 3 で curated/ に統合
