...
allowed-tools:
  ...
  - Bash(rm -f .state/dashboard.pid)
  - Bash(del .state\dashboard.pid)
...
**Mac / Linux / WSL**（Linux/WSL は `/proc`、macOS/BSD は `ps` フォールバックで identity 照合）:
```bash
PID=$(cat .state/dashboard.pid 2>/dev/null || true)
is_dashboard() {
  # identity: その pid の command line が dashboard/server.py を含むか。
  # Linux/WSL は /proc、macOS/BSD は ps で確認する（/proc が無くても照合できる）。
  if [ -r "/proc/$1/cmdline" ]; then
    grep -qa "dashboard/server.py" "/proc/$1/cmdline"
  else
    ps -p "$1" -o args= 2>/dev/null | grep -q "dashboard/server.py"
  fi
}
if [ -n "$PID" ] && kill -0 "$PID" 2>/dev/null && is_dashboard "$PID"; then
  kill -TERM "$PID" 2>/dev/null || true
  echo "dashboard (pid=$PID) stopped"
else
  echo "dashboard pid stale or not running; removing stale pidfile (no kill)"
fi
rm -f .state/dashboard.pid
```
このブロックの標準出力 1 行（`dashboard (pid=N) stopped` / `... stale or not running ...`）を
Step 4 の報告にそのまま転記する（実際に停止したか stale だったかを人間に正しく伝えるため）。
`/proc` も `ps` も無い環境（Windows native）は下の PowerShell を使う。
