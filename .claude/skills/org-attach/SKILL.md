...
description: >
  ...
  と `/usr/bin/tmux -L claude-org-broker list-panes -a`（pane_id ↔ session マッピング）を
...
> このスキルは tmux backend（socket `claude-org-broker`）上の **detached tmux session への attach** を
...
別物（プラグインのラッパ）が走り、`-L claude-org-broker` の socket 指定や `attach -r -t <session>` が
...
- `id`: tmux backend では **これが tmux の pane_id（`%N` 文字列）そのもの**。
  ...
  tmux socket `claude-org-broker` では `%N` 文字列で来る。）
...
> broker socket の detached session には**出現せず attach 対象外**である（窓口は org を起動した人間の
...
> この列挙は表示補助にしか使わないため、**attach 可否・join・lifecycle 判定のいずれにも波及させない**
> （本スキルは read-only であり、縮退が破壊的な後段に繋がる経路を持たない）。
...
- `-L claude-org-broker`: runtime の `BROKER_SOCKET`。組織の tmux はこの socket 上にある
...
出力例（1 行 = 1 ペイン、各 broker ペインは独立 session `claude-org-broker-{pid}-{seq}`）:
...
socket 接続自体が落ちている場合は `error connecting to ... claude-org-broker` が返る。その時は
「tmux socket `claude-org-broker` に繋がりません（組織が起動していない可能性）。`/org-start` 済みか
確認してください」と報告して停止する。
...
- **読取専用 attach（既定・安全）**: `/usr/bin/tmux -L claude-org-broker attach -r -t <session>`
...
`/usr/bin/tmux -L claude-org-broker list-panes -a -F '#{pane_id} #{session_name}'`
（secretary は logical pane なので broker socket に**出ない**点に注意）:
...
■ dispatcher (dispatcher)          session=claude-org-broker-1912-14
  読取専用 : /usr/bin/tmux -L claude-org-broker attach -r -t claude-org-broker-1912-14
  読取書込 : /usr/bin/tmux -L claude-org-broker attach    -t claude-org-broker-1912-14

■ worker (worker-feat-org-attach-skill)   session=claude-org-broker-1912-17
  読取専用 : /usr/bin/tmux -L claude-org-broker attach -r -t claude-org-broker-1912-17
  読取書込 : /usr/bin/tmux -L claude-org-broker attach    -t claude-org-broker-1912-17

(logical pane / attach 対象外)
- secretary (secretary): 窓口は logical pane（pane_id 無し・broker socket 非出現）。
...
- tmux session claude-org-broker-1912-9 (%9)  : broker 未登録 session（組織外/孤児の可能性）
