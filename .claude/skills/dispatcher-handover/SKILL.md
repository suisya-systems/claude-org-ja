...
>   - `.state/dispatcher-event-cursor.txt`（次サイクルの poll_events cursor。backend
>     session に束縛されており、resume 後の 1 サイクル目が束縛を照合してから使う）
...
- `.state/dispatcher-event-cursor.txt`: poll_events 次 cursor（`session_key` と対で保存されており、resume 後の 1 サイクル目が現 backend session と照合してから使う。不一致なら監視ループが破棄して張り直し `EVENT_CURSOR_RESET` を窓口へ報告する）
- `.state/dispatcher/worker-idle-state.json`: stall 検出の idle streak
