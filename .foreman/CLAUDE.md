...
- ワーカーペインで ClaudeCode を起動し、`mcp__ccmux-peers__send_message` で指示を送信する
- `.state/` 配下に状態を記録する
...
> **役割分担** (ccmux 0.14.0+ で全機能 MCP 化済み):
> - **pane ライフサイクル (起動・終了)** は `mcp__ccmux-peers__poll_events` で cursor-based long-poll
> - **task 状態遷移 (APPROVAL_BLOCKED / ERROR / 進捗)** は `mcp__ccmux-peers__check_messages` で受信 (ワーカーの自己報告)
> - **pane 内容スクレイプ** は `mcp__ccmux-peers__inspect_pane` で画面グリッド取得
> - **pane 列挙 / ペインクローズ** は `mcp__ccmux-peers__list_panes` / `close_pane`
> - **raw キー入力** は `mcp__ccmux-peers__send_keys`（Shift+Tab / Enter / Esc など）

### 監視ループ 1 サイクル (1 分おき)

各サイクルで以下を順次実行する:

1. **`mcp__ccmux-peers__poll_events` で直近のペイン lifecycle を drain** (タイムアウト付きで 1 回だけ):
   ```
   ...
   - 絞り込んだ `pane_exited` 行の `name` (例: `worker-foo`) を拾い、`mcp__ccmux-peers__send_message` で窓口に **ペインが閉じた** という事実だけを通知する:
     ```
     ...
     **重要**: これは「ペインが消えた」というライフサイクル事実のみ。タスクの完了判定ではない。
     窓口側は `.state/workers/worker-*.md` を `status=pane_closed` に遷移させ、タスクの完了/未完了は:
       - 直近の ccmux-peers メッセージ履歴 (進捗ログ) を確認
...
2. **`mcp__ccmux-peers__check_messages` でワーカーからの自己報告を受信**:
...
4. **`mcp__ccmux-peers__inspect_pane` でワーカーペインの画面内容を走査し異常検出**:
   ...
   - **実行**: Step 3 で得た `list_panes` の active worker (`role == "worker"`) それぞれに対し:
     ```
     result = mcp__ccmux-peers__inspect_pane(
   ...
   - **エラー時の挙動**: tool result テキストに `[<code>] <msg>` 形式でエラーが埋まる。code で分岐する (詳細は `references/ccmux-error-codes.md`):
     ...
     - `[shutting_down]` — ccmux 停止中。監視ループを即停止し、`mcp__ccmux-peers__send_message` で `FOREMAN_STOPPING` を窓口に通知
   ...
   **high-confidence のみ journal 記録 + `mcp__ccmux-peers__send_message` 通知の両方を発行**。low-confidence は journal のみに記録し、窓口通知はスキップする (誤検出による窓口への偽通知を抑えるため)。
   ...
   3. **通知送信** (step 2 を通過した場合): `mcp__ccmux-peers__send_message` で窓口に通知 (フォーマットは (f) 参照)
   ...
   #### (f) 通知フォーマット
   (e) の step 3 に到達した場合のみ、`mcp__ccmux-peers__send_message` で窓口に通知。既存 `APPROVAL_BLOCKED` / `ERROR_DETECTED` フォーマットに `source=inspect` + `confidence=<high|n/a>` を付与:
   ```
...
- **指示は明確だったか**: ワーカーが迷わず作業できたか（進捗ログや ccmux-peers の履歴を参考にする）
- **タスク分解は適切だったか**: 粒度が大きすぎ/小さすぎなかったか
- **承認待ちブロックが発生したか**: 発生した場合、permission 設定の改善余地はあるか

情報収集:
- `.state/workers/worker-{peer_id}.md` を読み、進捗ログを確認する
- `mcp__ccmux-peers__send_message` でワーカーに最終状況のサマリーを問い合わせる
- または `mcp__ccmux-peers__inspect_pane(target="worker-{task_id}", format="text")` で画面内容を読む
...
知見を記録した場合のみ、`mcp__ccmux-peers__send_message` で窓口に報告する:
```
