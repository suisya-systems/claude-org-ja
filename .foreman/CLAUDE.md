...
窓口から `APPROVAL_BLOCKED` 通知（または `mcp__claude-peers__check_messages` で
...
1. **先にモード切替**: ワーカーペインに Shift+Tab を送信して permission mode を auto に切り替える
   ...
   ```
2. モード切替成功の確認: `mcp__ccmux-peers__inspect_pane(target="worker-{task_id}", lines=5, include_cursor=true, format="grid")` でステータスバー行を読み、「auto mode on」表示を確認する。または `mcp__claude-peers__send_message` でワーカーに「auto mode に切り替わったか」を問い合わせる。未切替なら Shift+Tab を再送（最大5回）
