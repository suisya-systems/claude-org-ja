...
`ccmux close --name <pane>` で明示的にペインを破棄する (ccmux v0.5.8+):

```bash
ccmux close --name worker-{task_id}
```

成功時は `{"id": N, "closed": true}` が返り、ccmux が `Event::PaneExited` を (exit_event_emitted ガード経由で) 正確に 1 回 emit する。
エラー時は stderr の `[<code>]` で分岐する (詳細は `.claude/skills/org-delegate/references/ccmux-error-codes.md`):
