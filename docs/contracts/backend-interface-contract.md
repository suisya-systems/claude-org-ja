...
> **Proposed amendment (capability-conditional cross-tab addressing)**: **both** the first bullet below (the operational "all pane-addressed operations … MUST resolve only against panes in the current tab" rule, which names `send_message` explicitly) **and** the SINGLE-TAB MUST decision bullet are **conditionally superseded** by T-§4.2 of the [Proposed amendment (renga 2.0 multi-tab): capability-conditional cross-tab addressing](#proposed-amendment-renga-20-multi-tab-capability-conditional-cross-tab-addressing). The two halves are gated on **different, independent capabilities** (T-§cap): under **`cross_tab_peers`** the messaging scope diverges — names resolve inside the sender's tab, numeric peer ids are accepted across tabs, and peers are enumerated from every tab; under **`caller_scope`** the pane-control scope becomes **caller-tab-scoped and focus-independent**. A backend advertising only `cross_tab_peers` gets the messaging change while its pane-addressed control still follows the **focused** tab (ratified §4.3); a backend advertising only `caller_scope` gets caller-tab-scoped control while its messaging stays same-tab. In every case the pane-control scope stays **one tab wide**. Backends that do not advertise the capability, **including `org-broker`** (whose §4 broker note above therefore continues to read exactly as written), keep the ratified SINGLE-TAB MUST verbatim. Status: PROPOSED, pending human ratification; **nothing below is deleted or weakened**.
...
- **Capability set**: a backend MAY advertise a set of named capabilities. A capability name is an opaque lowercase identifier; the set is open (new names MAY be added at any time, in the same spirit as the §6.2 error-code rule). This amendment defines the meaning of three names only:
  ...
  - **`caller_scope`** — pane-addressed **control** operations (§1.x, `set_pane_identity`, spawn `target`) resolve against the **caller's** tab rather than the focused tab, and `list_panes` returns the caller's tab only. **This is the gate for the pane-control-scope rules** — specifically T-§4.2's focus-independent resolution of the literal `"focused"` and its conditional supersession of §4.3's visibility consequence. Because capabilities are **independent** (below), a backend MAY advertise one without the other, so the two gates MUST be evaluated separately: a backend advertising only `cross_tab_peers` gets the messaging rules while its pane-addressed operations keep following the **focused** tab per ratified §4.3, and a backend advertising only `caller_scope` gets focus-independent pane control while its messaging stays same-tab per ratified §2.1 / §2.2. A harness that has not established `caller_scope` MUST NOT assume `list_panes` reflects its own tab — with focus elsewhere it would inspect or close panes of another tab.
  - **`spawn_tab`** — spawn variants accept a tab selector. `spawn_tab` is the **canonical wire name** advertised and queried for this capability (renga's internal constant is spelled `CAP_SPAWN_TAB`; that is an implementation detail, not the advertised identifier). Naming it explicitly is required for the same reason as the other two: a capability that has no agreed identifier cannot be advertised, queried, or gated, and its `server_too_old` handling (T-§6) would be unreachable. The placement behaviour itself is out of scope for this amendment; only the identifier and its error codes are contracted here.
...
**Proposed (supersede, not delete) — the two halves are gated on *different* capabilities, per the independence rule of T-§cap:** the single "all pane-addressed operations" scope splits into two scopes that MUST be reasoned about separately, and **each half applies only when its own capability is advertised**. The messaging half is gated on **`cross_tab_peers`**; the pane-control half's focus behaviour is gated on **`caller_scope`**. A backend advertising only one gets only that half:

- advertising **`cross_tab_peers` alone** → the messaging rules below apply, but pane-addressed control keeps following the **focused** tab per ratified §4.3 (it is NOT caller-tab-scoped);
- advertising **`caller_scope` alone** → pane-addressed control becomes caller-tab-scoped and focus-independent, but messaging stays same-tab per ratified §2.1 / §2.2.

Gating both halves on `cross_tab_peers` would contradict the independence rule and leave a `caller_scope`-only backend's pane control undefined; a harness would then operate on the globally focused tab where the caller's tab was intended.
...
- **Pane-control scope** (`list_panes`, `close_pane`, `inspect_pane`, `send_keys`, `set_pane_identity`, `focus_pane`, and the `target` parameter of every spawn variant in §1.1–§1.3) — **gated on `caller_scope`, not on `cross_tab_peers`**: the scope stays **one tab wide** either way, and cross-tab pane-addressed control returns `pane_not_found` exactly as the ratified §4.2 sentence says. What `caller_scope` changes is **which** tab: with it advertised the resolving tab is the **caller's**, focus-independently; **without it the resolving tab is the focused one** (ratified §4.3), so a harness MUST NOT assume `list_panes` and the other control calls reflect its own tab — and in particular **MUST NOT read absence from such an enumeration, or a `pane_not_found` from such a call, as evidence that a pane has exited**, because a focus change alone produces both.
