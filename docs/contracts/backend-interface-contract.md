...
>
> **Effect before ratification**: this section is informational, and the ratified single-tab text above remains operative. A harness MAY implement the capability-conditional behaviour ahead of ratification, because the non-advertising path — the path every currently deployed backend takes — is byte-for-byte the ratified behaviour; implementing it changes nothing until a `cross_tab_peers`-advertising backend is actually driven.
>
...
- **What a legacy determination licenses — and what it does not**: concluding that a backend is non-advertising selects the *addressing rules* (name-based, current-tab) and nothing further. It **MUST NOT** be used as grounds to skip a harness-side check whose purpose is to establish that a peer belongs to the harness's own organisation. A backend MAY enumerate peers from other tabs — and therefore from other organisations — while still omitting the marker fields, so ownership is harness policy that is **independent** of capability detection and MUST be checked on its own terms.
...
- **Per-event identity**: every `pane_exited` MUST carry at least one identifier that joins to an enumeration the harness already holds — the pane identifier in the same id space as `list_panes`, and/or, for a peer-bearing pane, the peer identifier in the same id space as `list_peers`. Field **names** are deliberately not contracted here (backends spell them differently); what is contracted is that a join is possible **without** going through `name`.
...
This amendment is **PROPOSED**. Ratification is a human decision made on review of this contract change; no dogfood or spike gate is attached **to the ratification**, because at the time of drafting the amendment is behaviour-neutral for every deployed backend: `org-broker` advertises no capability named here, and the fail-safe default of T-§cap routes every undetermined case to the ratified rules. That neutrality is time-bound by construction — it holds exactly until a `cross_tab_peers`-advertising backend is driven, which is the situation the amendment exists for. A separate **operational** gate therefore applies and is *not* satisfied by ratification: the first drive of a capability-advertising backend MUST be run and reported as a dogfood step before the harness relies on cross-tab addressing in production. Until ratified, the ratified §2.1 / §2.2 / §4.2 / §4.3 text, the Q10 row, and cluster-summary item 4 remain operative for all backends.
