...
### I3 — Pane-liveness vs. run-status coherence (steady-state)
The coupling between `runs.status` and worker-pane liveness is a **steady-state** invariant — it holds in the absence of an in-flight transition. There are two narrow normal-system intervals where it is permitted to lapse:

- **T5 close gap**: between secretary's `update_run_status('<task>', 'completed')` write and dispatcher's subsequent `CLOSE_PANE` execution, the pane is still live while `runs.status` is already `completed`. This is the documented Set B T5 ordering (`.claude/skills/org-pull-request/SKILL.md:74–79, 92–100`; `docs/contracts/delegation-lifecycle-contract.md:84–90`).
- **/org-suspend → /org-resume window**: `/org-suspend` graceful-closes worker panes (`.claude/skills/org-suspend/SKILL.md:10, 134`) but leaves runs at `runs.status='in_use'`; `/org-resume` does not auto-re-spawn panes — it requires per-run human approval before issuing a fresh delegation (`.claude/skills/org-resume/SKILL.md:71–90`). The `in_use`-without-pane state therefore persists across `org_sessions.status` `SUSPENDED → ACTIVE` until the human authorizes resume.

Outside those two intervals the steady-state coupling holds: `runs.status='in_use'` with a missing `worker-{task_id}` pane is drift (modulo the resume window) and warrants the dispatcher's `WORKER_PANE_EXITED` reconcile path (Set B § 2 T7); `runs.status='completed'` whose worker-state file is not in `.state/workers/archive/` after the post-commit hook ran (`tools/state_db/writer.py:597`) is also drift. The intervals are a deliberately narrow exception rather than a general looseness: they do not authorize ignoring the coupling at arbitrary times.
...
`worker_dirs.lifecycle ∈ {active, scratch, archived, delete_pending}` is **not** a projection of `runs.status`. A Pattern A worker_dir typically remains `lifecycle='active'` across many runs. Pattern B worktrees and Pattern C ephemeral dirs are **removed** from the `worker_dirs` table on T5 close via `StateWriter.remove_worker_dir()` (`tools/state_db/writer.py:391–397`; `.claude/skills/org-pull-request/SKILL.md:92–105`); the `archived` and `delete_pending` lifecycle values exist as enum slots for separate curator-driven sweeps (e.g., cold-completed-hot dirs older than 90 days, gc batches), not for the regular T5 close path. The `StateWriter` does not couple `runs.status` and `worker_dirs.lifecycle` mutations — each is its own call site, and the run row's worker_dir association becomes `NULL` after a `remove_worker_dir()` (`worker_dir_id REFERENCES worker_dirs(id) ON DELETE SET NULL`, `tools/state_db/schema.sql:84`).
