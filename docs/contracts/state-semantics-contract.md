...
>
> **Method**: Each statement below is sourced from the implementation as it exists today (post-M4 state-db cutover, Issue #267 / #284). Where the established prose contracts (Set B § 1, Set C § 1.1) or `docs/org-state-schema.md` carry an older view, the divergence is named explicitly so the reconciliation work tracked under Epic #357 can refer to the gap by name.
>
...
### I3 — Pane-liveness vs. run-status coherence
While `runs.status='in_use'`, the worker pane named `worker-{task_id}` MUST be open. While `runs.status='completed'`, the worker-state file MUST be in `.state/workers/archive/` (post-commit hook in `tools/state_db/writer.py:597`). The two are coupled: a `completed` run with a live pane, or an `in_use` run with no pane, indicates drift.

### I4 — Session vs. run independence
`org_sessions.status='SUSPENDED'` does NOT propagate to `runs.status`. Per Set B § 4.3, individual runs remain `in_use` across `/org-suspend`; the discrimination of "paused" vs. "active" is recovered from the worker's most recent SUSPEND report + Progress Log, not from a run-level status flip. The reserved-but-unused `runs.status='suspended'` value (§ 2) is **not** the persistence target for `/org-suspend` checkpoints today.
