...
- the binding applies to `claude-org-runtime` **0.1.39 through <0.2**. The lower bound is where the ledger landed, and it is deliberately *not* ja's dependency floor: `>=0.1.37,<0.2` ([`pyproject.toml`](../../pyproject.toml):30, [`requirements.txt`](../../requirements.txt):111) and the container default `ARG RUNTIME_VERSION=0.1.37` ([`docker/Dockerfile`](../../docker/Dockerfile):65) both predate it — `_seed_status`, `count_unbound_reservations` and `WORKER_BIND_WINDOW_SECONDS` are all absent from the 0.1.37 and 0.1.38 wheels (verified by unpacking both). On those runtimes no seed is ever read and this section simply does not apply, which is why the §7.5 guard skips rather than fails there. A runtime upgrade that moves the reader is a trigger to re-read this section, not an automatic contract change.
