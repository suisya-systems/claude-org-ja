...
- **Update cadence**: append-only, per event. Writes MUST go through `tools/journal_append.sh` or `tools/journal_append.py` (both a raw `>>` append and a hand-crafted `INSERT INTO events` are forbidden by Set A constraints — the helper owns timestamp generation, JSON escaping, and reserved-key checking).
- **Append-only obligation**: CONVENTION-ONLY. Appends MUST go through `tools/journal_append.{sh,py}`; hand-written appends / inserts are forbidden. Manual corrections during `/org-retro` are permitted ONLY to correct factual errors, and the corrected record MUST carry a reserved `_correction` field (JSON string) inside its `payload_json` recording the rationale, so the rationale stays in-band and the record stays machine-readable to `dashboard/server.py` and to SQL readers. (Pre-M4 this clause was phrased as "on the same line … no JSONL-illegal inline comments"; the JSONL-legality rationale is what the DB structure supersedes, not the in-band-rationale requirement itself.) The contract does NOT impose storage-level enforcement (no chattr `+a`, no checksum chain, no DB trigger). Per-record schema versioning beyond the `occurred_at` / `kind` envelope is not introduced; see §4.1.
...
3. **Append-only obligation for journal events** (§1.3 — the `events` table in `.state/state.db` since the M4 cutover; `.state/journal.jsonl` pre-M4) — Convention-only. Helper-mediated appends are mandatory; retro corrections are permitted with an in-band marker. No storage-level enforcement.
