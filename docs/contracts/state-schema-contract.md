...
- **Schema**: `{"version": 1, "session_key": string|null, "session_key_source": string, "next_since": string}`.
  ...
  - `session_key_source` — how `session_key` was derived, and therefore how a reader must treat a `null`: `"server_info.session_id"` (exact — the backend's per-process-instance identifier), `"pid_endpoint"` (approximation `pid={pid};endpoint={endpoint}` for backends whose `server_info` omits the session identifier), `"transport_has_no_server_info"` (the transport has no `server_info` at all — `org-broker` today). A fourth state, `"unbindable"` (the surface exists but returned no provenance this cycle), is never **written**: a cycle that cannot establish provenance leaves the file untouched, so an unverifiable cycle neither reuses nor destroys the last known binding.
- **Binding rule (normative)**: `next_since` is a **counter internal to one backend session** — it is not restored across a daemon restart, while this file is. A reader MUST therefore use a stored `next_since` only when the stored `session_key` is non-`null` and equals the session key observed in the current cycle. The sole exception is `session_key_source == "transport_has_no_server_info"`, where no binding can be established at all and the stored cursor is used as before — and that exception applies only when the **stored** `session_key_source` carries the same value, so a cursor issued by one transport is never handed to another. On any other mismatch — including a legacy plain-text file with no JSON envelope — the reader MUST discard the cursor, restart from present-time semantics, and report the discard rather than absorb it (`anomaly_observed` / `notify_sent` with payload `kind` `stale_event_cursor` **and payload `session_key` plus `discarded_session_key`**, plus an `EVENT_CURSOR_RESET` line to the secretary; de-duplication keyed on both values is what keeps a later restart — or a second episode in which no key could be established at all, where `session_key` alone is `null` every time — from being suppressed by the earlier report, and `notify_sent` is written only after the send succeeds, so a failed send stays visible as an `anomaly_observed` row with no matching `notify_sent` and is retried on a later cycle). An absent or empty file is a **start, not a discard**, and MUST NOT be reported; likewise a cycle that could not establish provenance at all is an **unverifiable cycle, not a discard** — the writer leaves the file alone, the reader does not reuse the cursor for that cycle, and the lifecycle surface is reported as unobservable instead. Confining discards to actual mismatches is what keeps both sides of the de-dup key populated with real values, so a later mismatch is never suppressed by an earlier one. Silently reusing a cross-session cursor makes every subsequent `pane_exited` miss while the watch loop keeps running, which is a safe-side degradation that looks healthy from outside. The procedure is `.dispatcher/references/worker-monitoring.md` § "監視ループ 1 サイクル" Step 1; the same discipline for pane ids is Set T's T-§4.2-id (O1).
...
## 6. Amendment — SQLite state DB (`event_deliveries` outbox ledger, Refs #653 #658)

> **Status**: Additive amendment (2026-07-13). Does NOT rewrite any ratified §1–§5 text above; it extends the inventory to record the SQLite state DB, which landed after the 2026-05-03 ratification (M0–M4, Issue #267) and was not enumerated in §1.

The `.state/state.db` **SQLite** database is the machine-tractable projection of the state surface (M0–M4, Issue #267). Its schema SoT is `tools/state_db/schema.sql` (fresh-DB construction) plus per-table forward-migration DDL constants + `ensure_*_schema()` functions in `tools/state_db/__init__.py` (in-place upgrade of existing DBs). Migration bookkeeping lives in the DB's own `schema_migrations` table (integer `version` + description), **distinct** from the JSON-file `version` field of §4.1 and from `tools/state_migrate.py` (§4.4, which governs `.state/*.json` files only — the SQLite DDL migrations are a separate layer and MUST NOT be routed through it).
...
- the binding applies to `claude-org-runtime` **0.1.39 through <0.2**, which as of #841 coincides with ja's dependency floor (`>=0.1.39,<0.2` — [`pyproject.toml`](../../pyproject.toml):30, [`requirements.txt`](../../requirements.txt):123) and the container default (`ARG RUNTIME_VERSION=0.1.39` — [`docker/Dockerfile`](../../docker/Dockerfile):65). The coincidence is not the reason for the bound: 0.1.39 is simply where the ledger landed. `_seed_status`, `count_unbound_reservations` and `WORKER_BIND_WINDOW_SECONDS` are all absent from the 0.1.37 and 0.1.38 wheels (verified by unpacking both), so on such a runtime no seed is ever read and this section does not apply — which is why the §7.5 guard skips rather than fails below the bound, and why it keeps that branch even though the pin no longer admits those versions. A runtime upgrade that moves the reader is a trigger to re-read this section, not an automatic contract change.
