...
- **Re-apply**: `queued` is the only status apply may re-apply onto (a correction of a not-yet-sent delegation); it updates the reservation and does **not** append a second `delegate_sent`, so the event is exactly-once per `task_id` — including a `delegate_sent` written by an older hand-typed `journal_append`, which carries no run link. Apply refuses a `task_id` whose run is in any other status — a new delegation needs a new `task_id`. It also refuses when the re-applied plan resolved to a different project / pattern / branch / worker dir than the reservation holds, because moving the reservation would leave it disagreeing with the `dir` already journalled (the first reservation counts as active for the next layout resolution, so this flip can happen from an unchanged command). A failure before the final transaction reserves nothing and journals nothing, so the same `task_id` stays re-appliable.
