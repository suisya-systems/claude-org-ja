...
> **Status**: Ratified (2026-05-03); amended 2026-06-07 (§2.2 R2 trigger — Set A Q10 amendment, curator on-demand: threshold judgment externalized to `tools/check_curate_threshold.py`, resident `/loop 30m` retired); amended 2026-07-22 (Issue #755 — operator-private local-asset split: the skill-candidate queue's real entries and operator-private curated notes are machine-local and gitignored, while the referenced operational curated docs and the entry-format definition stay public; see §1.4 and §3.4). Lead-confirmed decisions for all 9 open questions. This contract defines the knowledge-and-curation surface that the `claude-org` harness reads and writes.
>
...
### 1.4 `knowledge/skill-candidates.md` (+ machine-local `knowledge/skill-candidates.local.md`)

- **Path**: split into two files (Issue #755). `knowledge/skill-candidates.md` is the **tracked, public** file and carries the entry-**format** definition, `status` vocabulary, and operational rules only — its `## エントリ一覧` list is always empty. `knowledge/skill-candidates.local.md` is the **machine-local, gitignored** sibling that holds the real `skill_recommend` entries, which are operator-private working knowledge and MUST NOT be committed to the OSS repo. `tools/check_curate_threshold.py`'s `CANDIDATE_ENTRY_PATHS` is the single source of truth for the two-file read order `[public, local]`; `count_pending` sums both (each with independent code-fence state, a missing file counting as 0), and `skill-audit` Step 1's awk reads the same two files in the same order (three-way sync asserted by `tools/test_check_curate_threshold.py`).
...
- **Schema**: `status` ∈ `{pending, deferred, approved, rejected, merged-into-{existing-skill}}`. `deferred` (Issue #753) is the "presented to the human, human chose to hold" state: **not terminal** (the candidate may later be revisited) but **excluded from the threshold count and never re-asked**, distinct from the terminal `approved` / `rejected` / `merged-into-*`. Once an entry leaves `pending` (including to `deferred`), it is retained as history (not deleted) per the file's "運用メモ" section.
- **Owner**: `skill-eligibility-check` Step 4 (auto-append on `skill_recommend` decisions) — entries are appended to the machine-local `knowledge/skill-candidates.local.md`, never to the public file (Issue #755). Status transitions (`pending` → `deferred` / `approved` / `rejected` / `merged-into-*`, plus `決定日` / `却下理由` / `統合先` fields) are written by the worker that performs the corresponding promotion task (per the §2.4 execution-path decision) on the same `.local.md` file; the `pending` → `deferred` update on a human "hold" decision follows the same delegated-write path. The curator does NOT directly hand-edit either file; its writes happen through `skill-eligibility-check`. The secretary does NOT edit either file in place; for human-relayed batch decisions, the secretary delegates the status update to a worker alongside the skill creation. The public `knowledge/skill-candidates.md` is edited only when the entry-format definition or operational rules themselves change (a documentation edit), never to add candidate entries.
- **Readers**: secretary (batch question to the human when the combined `pending` count across both files `≥ 5`, per Issue #68 batch-rationale cited in the file header), `skill-audit` Step 1 (counts `pending` entries across both files — `deferred` and terminal statuses are excluded by the exact `- **status**: pending` match — to decide whether to fire), the curator (re-read during the next curation cycle to detect already-queued patterns; the real entries it reads live in `.local.md`).
- **Lifecycle**: append-only at the entry level. Same `pattern_name` while still `pending`: existing entry's `関連タスク` / `関連 raw ファイル` are merged in (no new entry). Same `pattern_name` after a terminal status (`approved` / `rejected` / `merged-into-*`): a new dated entry is added so the prior decision and rationale survive as history (a legitimately recurring pattern is re-surfaced, not suppressed). `deferred` is the one exception: a `deferred` entry is never moved back to `pending` and the same `pattern_name` is NOT auto-re-appended — that is precisely what stops a shelved candidate from re-firing the curator threshold on every worker close; a new dated entry is added only if the human explicitly chooses to revisit it.
...
- **Effect**: 5-signal scoring per `references/signals.md`; decision ∈ `{skill_recommend, candidate_queue, curated_only}`. On `skill_recommend`, Step 4 auto-appends to the machine-local `knowledge/skill-candidates.local.md` (or merges into an existing `pending` entry of the same `pattern_name` there), never to the public `knowledge/skill-candidates.md`, per the §1.4 public/local split (Issue #755).
...
- **Trigger**: the combined `pending` count across `knowledge/skill-candidates.md` + `knowledge/skill-candidates.local.md` crosses N=5 (per `skill-audit` Step 1 and the Issue #68 batch-question rationale; entries live in the `.local.md` file per §1.4). Secretary batch-prompts the human. On human approval, the secretary delegates skill creation (`.claude/skills/{name}/SKILL.md` from `.claude/skills/org-retro/references/work-skill-template.md`) and the corresponding `knowledge/skill-candidates.local.md` status update to a worker via `org-delegate`, per the §2.4 execution-path decision below.
- **Effect**: new skill file under `.claude/skills/{name}/`; the `knowledge/skill-candidates.local.md` entry transitions to `approved` (with `決定日`) or `rejected` (with `却下理由`). `merged-into-{existing-skill}` is used when the candidate's value is folded into an existing skill rather than creating a new one. When the human chooses to hold rather than decide, the entry transitions to `deferred` (§1.4) — no skill file is created; the candidate simply stops counting toward the N=5 threshold and is not re-asked.
...
- **Execution path**: skill creation (writing `.claude/skills/{name}/SKILL.md` from the work-skill template, plus the corresponding `knowledge/skill-candidates.local.md` status transition) is delegated to a worker task via `org-delegate`, consistent with Set A's principle "実作業は全てワーカーに委譲する". The worker write-surface carve-out for this task is recorded in §3.1. The secretary MUST NOT edit skill files in place, and the dispatcher MUST NOT write skill files even when the candidate originated from `org-retro` Step 4.2; in that case the dispatcher's role is limited to recording the queue transition through `skill-eligibility-check`'s normal append path (which targets `knowledge/skill-candidates.local.md`). The current `.claude/skills/org-retro/SKILL.md` Step 4.2 still has the dispatcher creating the SKILL.md and updating the `skill-candidates.local.md` status directly; this is non-conformant with the contracted execution path and is tracked as a follow-up Issue ("feat(org-retro): hand skill-promotion execution off to a delegated worker, not the dispatcher itself").
...
- **MAY write (delegated promotion task only)**: `.claude/skills/{name}/SKILL.md` (and surrounding files under `.claude/skills/{name}/`) AND the corresponding `knowledge/skill-candidates.local.md` status transition (the entry's `status` / `決定日` / `却下理由` / `統合先` fields — entries live in the machine-local `.local.md` file per §1.4, never in the public `knowledge/skill-candidates.md`), but ONLY when the worker's brief explicitly delegates a §2.4 skill-promotion task. Outside such a brief, these paths remain forbidden. Set A § Role: worker reconciled this carve-out into its `Outputs > Files written` list (issue #251).
- **MAY write (delegated scrub task only)**: `knowledge/raw/{YYYY-MM-DD}-{topic}.md` and files under `knowledge/raw/archive/`, when the brief delegates the §3.4 scrub of operator-private content from those paths. The scope is strictly the redaction described in the brief; no other edits, and no writes under `knowledge/curated/` (which remains curator-only per §1.3 / §3.2 — curated-content scrubs route to the curator per §3.4, not to a worker).
- **MUST NOT write (otherwise)**: `knowledge/curated/`, `knowledge/raw/archive/`, `knowledge/skill-candidates.md`, `knowledge/skill-candidates.local.md`, `.claude/skills/`. In particular the public `knowledge/skill-candidates.md` (format-definition only, entry list always empty per §1.4) is never a worker write target; the `.local.md` sibling is writable only under the delegated-promotion carve-out above. Workers also cannot reproduce the `knowledge/` directory inside their `worker_dir` (per Set A § Role: worker constraints).
- **MAY read**: `knowledge/curated/` and `knowledge/raw/` as reference material (per Set A § Role: worker reads).

### 3.2 Curator boundary

- **MAY write**: `knowledge/curated/`, `knowledge/raw/archive/` (move-target, plus annotations such as the `<!-- curated -->` marker applied to entries already moved into `archive/`), `knowledge/skill-candidates.local.md` indirectly via `skill-eligibility-check` (entries live in the `.local.md` file per §1.4; the curator never writes the public `knowledge/skill-candidates.md`). Per the move-then-mark rule in §1.1, the curator MUST NOT mutate entries in the active `knowledge/raw/` set.
...
- **Stance (amended 2026-07-22, Issue #755)**: shareable **where tracked**, with a defined operator-private subset held as machine-local, gitignored assets. The publication boundary is:
  - **Public / tracked**: the operational curated docs `knowledge/curated/*.md` (without a `.local` suffix) that worker briefs and skills reference, and the entry-format definition in `knowledge/skill-candidates.md` (entry list always empty). These are treated as publishable artifacts — removing them would break `git`-fresh clones (worker / CI reference cuts).
  - **Machine-local / gitignored (operator-private)**: `knowledge/raw/` (worker/dispatcher raw learnings — already gitignored, machine-local scratch), operator-private curated notes named `knowledge/curated/*.local.md`, and the real skill-candidate entries in `knowledge/skill-candidates.local.md`. These never enter the OSS repo. History rewrite of any operator-private content already committed in the past is out of scope for this contract.
- **Forbidden content**: operator-private content (operator names, internal-system identifiers, customer data, secrets, internal URLs, and any other content the operator would not publish to a public repository) MUST NOT appear in any **tracked** file — neither the public curated docs nor the format-definition `knowledge/skill-candidates.md`. It MAY appear only in the machine-local, gitignored surfaces enumerated above.
- **Worker obligation**: workers MUST be informed of this no-operator-private-content rule via their delegation brief. The `org-delegate` instruction template MUST carry the rule; the template does not yet do so today, and the update is tracked as a follow-up Issue ("docs(skills): add 'no operator-private content in knowledge/' guidance to org-delegate instruction template"). Until the template is updated, the secretary MUST add the rule to each delegation brief individually.
- **Scrub ownership**: on discovery of operator-private content in a **tracked** file under `knowledge/` (the machine-local surfaces in the Stance above are permitted to hold such content and are not scrub targets), the secretary owns triage and routes the scrub to the role that holds write authority for the affected path:
  - `knowledge/raw/` or `knowledge/raw/archive/` content that is somehow tracked (raw is normally gitignored/machine-local) → delegated to a worker under the §3.1 "delegated scrub task" carve-out.
  - tracked `knowledge/curated/*.md` content → delegated to the curator (the only role with write authority on `knowledge/curated/` per §1.3 / §3.2). The secretary issues the scrub request via renga-peers; the curator performs the redaction in its next cycle.
