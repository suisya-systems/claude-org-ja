...
> **Status**: Ratified (2026-05-03); amended 2026-06-07 (Q10 — curator residency replaced by the on-demand worker-close trigger, human-approved in the curator-on-demand change; see § Role: curator); amended 2026-07-05 (curator spawn model `opus` → `sonnet` — auto mode's safety classifier runs on a dedicated model independent of the session model, and the curation workload is lightweight/mechanical; worker model wording synced to the current default-opus / lightweight-Sonnet policy; see § Role: curator and § Role: worker); amended 2026-07-27 (journal surface — the retired `.state/journal.jsonl` replaced by the `events` table in `.state/state.db` per the M4 cutover; the helper-only write obligation is unchanged and now also forbids direct `INSERT`; see § Role: secretary Inputs / Outputs / Hard prohibitions and § Role: dispatcher Outputs); amended 2026-07-27 (Codex model pin — `gpt-5.5` → `gpt-5.6-sol`, the current-generation name executable on a ChatGPT account, with the pin rationale generalized from the `-codex`-suffix case to per-name verification on each generation bump; see § Role: worker Constraints); amended 2026-08-29 (worker model selection — the Sonnet exception is scoped by oracle strength and decision density rather than by task weight: `sonnet` is allowed, on the secretary's explicit designation, for tasks that both pin a machine-checkable ground truth outside the implementation (fixtures, a test suite, database rows) and carry design decisions already fixed in the brief, while `opus` stays the default and is kept for tasks that re-derive a mechanism, settle a contract, or carry decisions that can only be made from measurement; human-approved; see § Role: worker Constraints and Lifecycle); amended 2026-09-06 (Codex model pin — `gpt-5.6-sol` → `gpt-6-astra`, the current-generation name executable on a ChatGPT account, re-measured before the swap per the standing per-name verification rule; a codex-cli floor is contracted alongside it because `gpt-6-astra` is rejected by older CLIs with a version error distinct from an unknown-name 400; see § Role: worker Constraints). Lead-confirmed decisions for all open questions. This document specifies the four roles (`secretary`, `dispatcher`, `curator`, `worker`) as they exist in the current `claude-org` implementation.
>
...
- **MCP calls** — `spawn_claude_pane` (workers, `model="opus"` by default — `sonnet` only for oracle-strong tasks the secretary explicitly designates — `permission_mode=auto`), `send_keys(enter=true)` for the dev-channel prompt, `list_peers` polling, `send_message` for the instruction, `inspect_pane` for the watch loop, `close_pane` for teardown.
...
- **Files written**:
  ...
  - For skill-promotion delegations specifically (per Set E §2.4 / §3.1 carve-out), the worker MAY write to `.claude/skills/{skill_name}/` (creating new skill files or editing existing ones) AND apply the corresponding `knowledge/skill-candidates.md` status transition (`status` → `deferred` / `approved` / `rejected` / `merged-into-*`, plus the `決定日` / `却下理由` / `統合先` fields for the same entry; `deferred` is the human-hold transition per Set E §1.4, Issue #753). This is a delegated-task scope, not a general worker capability; the worker MUST NOT write to `.claude/skills/` or to `knowledge/skill-candidates.md` outside an explicitly skill-promotion-tagged delegation.
...
- **Permission mode**: `auto` (hardcoded literal at spawn sites; `registry/org-config.md` value is reference-only — see its sync-warning). Model: **`opus` by default** (quality-first) — `sonnet` is permitted only for oracle-strong tasks the secretary explicitly designates, meaning tasks that satisfy **both** (a) a machine-checkable ground truth pinned outside the implementation (fixture expectations, test-suite assertions, database rows) and (b) design decisions already fixed in the brief, with none left to settle during implementation. The former rationale ("`auto` mode's safety classifier is only stable on Opus") is obsolete: the classifier runs on a dedicated model independent of the session model (https://www.anthropic.com/engineering/claude-code-auto-mode), so the opus default is a quality decision, not a classifier constraint (see `.dispatcher/references/spawn-flow.md` Step 3-2).
...
- **Codex discipline** (full mode, when `codex` available):
  - Diff self-review uses the **review surface**: `codex exec review --base origin/main -m gpt-6-astra -c model_reasoning_effort=medium` (pass the branch's base branch to `--base`; use the remote-tracking `origin/main`, not local `main`, so a stale local `main` in a shared clone can't drag unrelated task diffs into the review — run `git fetch origin` once before referencing it) (≈2× faster than the old `codex exec`-direct heavy-prompt form on small/medium diffs, at parity on safety-critical findings). The `codex:rescue` skill remains forbidden (past 18-min hangs). Operational guards and the speed/coverage caveats are the SoT of `knowledge/curated/codex.md`; those speed/coverage figures were measured on the `gpt-5.5` generation and have not been re-measured on the pinned model, so they bind as guards, not as claims about the current model.
  ...
  - Model is pinned explicitly via `-m` because a ChatGPT account accepts only certain model names; the current-generation executable name is `gpt-6-astra`. Acceptance is per name, not per generation number and not per suffix (bare `gpt-6`, `gpt-6-codex`, and `gpt-6-sol` all return 400 while the previous generation's `gpt-5.6-sol` still runs), and the API-key surface is not executable, so a generation bump must be confirmed by an actual invocation before the pin is changed. `gpt-6-astra` additionally requires **codex-cli >= 0.153.4** (0.147.0 rejects it with a distinct "requires a newer version of Codex" 400 rather than an unknown-name 400; the first accepting version between the two is unverified). The measured availability matrix and the CLI floor are the SoT of `knowledge/curated/codex.md`.
...
- **Spawn**: By the dispatcher in `org-delegate` Step 3, via `mcp__renga-peers__spawn_claude_pane(role="worker", name="worker-{task_id}", cwd={worker_dir}, permission_mode=auto, model="opus")` (model is the opus default; `sonnet` only when the secretary explicitly designates an oracle-strong task per § Role: worker Constraints, signalled by a `モデル: sonnet` line the secretary adds to the DELEGATE body — absent that line the dispatcher spawns with `opus`) after balanced-split target/direction selection. CLAUDE.md and `settings.local.json` are placed by the secretary in Step 1.5 *before* spawn.
