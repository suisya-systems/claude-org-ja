...
- **Codex discipline** (full mode, when `codex` available):
  - Diff self-review uses the **review surface**: `codex exec review --base origin/main -m gpt-5.6-sol -c model_reasoning_effort=medium` (pass the branch's base branch to `--base`; use the remote-tracking `origin/main`, not local `main`, so a stale local `main` in a shared clone can't drag unrelated task diffs into the review — run `git fetch origin` once before referencing it) (≈2× faster than the old `codex exec`-direct heavy-prompt form on small/medium diffs, at parity on safety-critical findings). The `codex:rescue` skill remains forbidden (past 18-min hangs). Operational guards and the speed/coverage caveats are the SoT of `knowledge/curated/codex.md`.
  ...
  - Model is pinned explicitly via `-m` because a ChatGPT account accepts only certain model names; the current-generation executable name is `gpt-5.6-sol`. Acceptance is per name including suffix, not per generation number (bare `gpt-5.6`, `gpt-5.6-codex`, and `gpt-5.5-codex` all return 400), and the API-key surface is not executable, so a generation bump must be confirmed by an actual invocation before the pin is changed. The measured availability matrix is the SoT of `knowledge/curated/codex.md`.
