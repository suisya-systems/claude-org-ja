...
- Foreman / Curator 起動直後の「開発チャネル確認プロンプト」を `mcp__renga-peers__send_keys(target=<pane>, enter=true)` で Enter 注入して通過している（`org-start` SKILL Step 2 / Step 3 の手順）
...
- renga-peers で通信できない → `claude mcp list` で `renga-peers` が Connected か確認、`list_peers` で peer ID（`worker-{task_id}` / `foreman` / `curator` / `secretary`）を確認
