...
| プロジェクト | 協調方式 | 詳細 |
|---|---|---|
| **claude-org** | **階層型＋P2P** | Secretary→Foreman→Worker の階層的委譲 ＋ `ccmux-peers` MCP でのP2P通信（同タブスコープ） |
...
| プロジェクト | 通信方式 | 特徴 |
|---|---|---|
| **claude-org** | **`ccmux-peers` MCP（同タブ内 P2P プッシュ型）＋ CLAUDE.md（永続ベースライン）** | **二重化による信頼性。揮発的通信 ＋ 永続的指示の組み合わせ** |
...
**claude-orgの独自性**: **指示の二重化**（CLAUDE.md永続指示 ＋ `ccmux-peers` リアルタイム通信）は他に類を見ない設計。Agent Teams のメールボックスシステムが最も近い。
