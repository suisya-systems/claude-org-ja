...
| **`.state/org-state.md`**（derived）| repo-relative | 人間可読スナップショット（ダッシュボードや /org-resume の参照対象） | `StateWriter.transaction()` の post-commit hook が `tools/state_db.snapshotter` で自動再生成。手動編集は `tools/state_db.drift_check` が drift 検出 |
...
│   ├── org-state.md               # state.db から自動再生成される派生スナップショット
│   ├── org-state.json             # state.db から生成される JSON 投影（dashboard 等向け）
│   └── workers/
