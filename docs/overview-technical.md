...
| **Foreman** | Yes | ペイン起動・指示送信・状態記録の代行 | Bash, Read, Write, Edit, Glob, Grep, Skill, ccmux-peers |
| **Curator** | Yes | `/loop 30m /org-curate` で知見整理 | Read, Write, Edit, Glob, Grep, Skill, ccmux-peers |
| **Worker** | No | 実作業（コード編集、調査、テスト等） | Bash, Read, Write, Edit, Glob, Grep, Agent, Skill, ccmux-peers |

### 通信

- **インスタンス間**: `ccmux-peers` MCP（同タブ内 Claude 間の双方向メッセージング、プッシュ型チャネル通知）。`send_message` / `list_peers` / `check_messages` / `set_summary` を用い、peer ID にはペイン名（`secretary` / `foreman` / `curator` / `worker-{task_id}`）を使う
- **ペイン管理**: `ccmux-peers` MCP（`spawn_pane` / `close_pane` / `list_panes` / `new_tab` / `focus_pane` / `inspect_pane` / `send_keys` / `poll_events` 等）。raw キー入力（`ccmux send --text`）・画面スクレイプ（`ccmux inspect`）・pane lifecycle イベント購読（`ccmux events`）の 3 機能は一部の references で CLI 併用が残存（upstream happy-ryo/ccmux#116 / #117 / #118 の MCP 対応完了まで）。MCP 側の等価ツールは出揃っているため、後続 Issue #30 で skill references を順次 MCP に置換予定
- **指示の二重化**: CLAUDE.md（永続・ベースライン）+ `ccmux-peers` メッセージ（リアルタイム・補足）
...
- **ターミナル/マルチプレクサ**: ccmux (ペイン分割で複数インスタンスを管理)
- **インスタンス間通信**: `ccmux-peers` MCP server（同タブ内の Claude Code 間メッセージング + ペイン制御を統合）
- **バージョン管理**: Git + GitHub (private repo: happy-ryo/aainc)
...
| `org-delegate` | 実作業が発生する依頼時 | Secretary → Foreman |
