...
| **Foreman** | Yes | ペイン起動・指示送信・状態記録の代行 | Bash, Read, Write, Edit, Glob, Grep, Skill, claude-peers |
| **Curator** | Yes | `/loop 30m /org-curate` で知見整理 | Read, Write, Edit, Glob, Grep, Skill, claude-peers |
| **Worker** | No | 実作業（コード編集、調査、テスト等） | Bash, Read, Write, Edit, Glob, Grep, Agent, Skill, claude-peers |

### 通信

- **インスタンス間**: claude-peers MCP（インメモリ、プッシュ型チャネル通知）、および同タブ内 Claude 間は ccmux-peers MCP
- **ペイン管理**: ccmux-peers MCP（`mcp__ccmux-peers__spawn_pane` / `close_pane` / `list_panes` / `new_tab` / `focus_pane` 等）。raw キー入力（`ccmux send --text`）・画面スクレイプ（`ccmux inspect`）・pane lifecycle イベント購読（`ccmux events`）の 3 機能のみ CLI 併用（upstream happy-ryo/ccmux#116 / #117 / #118 の MCP 対応完了まで）
- **指示の二重化**: CLAUDE.md（永続・ベースライン）+ claude-peers メッセージ（リアルタイム・補足）
...
- **インスタンス間通信**: claude-peers MCP server
...
| 指示の二重化 | CLAUDE.md + claude-peers メッセージ | 揮発的通信のみへの依存を回避 |
