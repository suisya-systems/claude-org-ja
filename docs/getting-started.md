...
- **ディスパッチャーで `missing required hook` に `check-loop-directive.sh` が出る場合**: `/loop` 監視ループの武装（`CronCreate` / `ScheduleWakeup`）を canonical 正文に固定するガードが配布されていない。この状態では自己流の短縮ディレクティブで `/loop` が張れてしまい、relay 実配送（`--list` → `send_message` → `--mark-delivered`）が落ちた監視ループが黙って回り続ける（2026-09-02 の事故）。窓口の Claude Code で `/org-setup` を再実行して `<repo>/.dispatcher/.claude/settings.local.json` に `matcher: "CronCreate|ScheduleWakeup"` の PreToolUse ブロックを配り直す（additive-only なので既存設定は壊れない）。既に別の drift も抱えている端末では `python tools/org_setup_prune.py --role dispatcher` で baseline へ丸ごと戻す。ガードの判定方針は [`.dispatcher/references/loop-directive-guard.md`](../.dispatcher/references/loop-directive-guard.md)
...
| `/org-curate` | 知見の整理 | 知見が溜まると自動で一時実行。手動でも可 |
...
2. 作業の区切り（ワーカー終了時）に自動で整理される（5件以上溜まったとき）
