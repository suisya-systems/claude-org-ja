...
| フォアマン（Foreman） | `bypassPermissions` | Sonnet | **例外**。理由は下記 |
...
2. **PreToolUse フック**（`bypassPermissions` でも実行され、exit code 2 でツール呼び出しをブロックできる。allow ルールより優先される）。**ただし現状フォアマンには PreToolUse フックが未配備**で、これは既知の改善項目として別 Issue で追跡予定です
3. **ロール契約による自主規律**（フォアマンの `CLAUDE.md` に記載された業務スコープ宣言と、窓口によるライフサイクル監視）

つまり「フォアマンのスコープはペイン起動・指示送信・状態記録に絞られているため事故クラスが限定的」という言い方は、**技術的境界ではなくロール契約に基づく自主規律**であることを正直に認める必要があります。技術的には bypass モードのフォアマンが `git push --force` を呼ぶ経路は止まりません（PreToolUse フックを配備するまでは）。トレードオフは伏せずに開示します。

**代替手段**: 各ロールごとに `tools/role_configs_schema.json` を正典とした `settings.local.json` を `/org-setup` で配布します。許可エントリはスキーマに登録し、CI で設定の乖離を検出します。一時的に制約を緩める必要があれば、対象スコープを限定したフックや `permissions.allow` の追記をスキーマ経由で行ってください。フォアマンの `bypassPermissions` 例外を撤廃したい場合は、Sonnet で `auto` モードが利用可能になった時点で再評価します。それまでの実効防御強化として、フォアマン用 PreToolUse フック配備（`git push --force` 系・破壊的 `git` 操作・任意ファイル編集の遮断）を別 Issue で追跡予定です。
