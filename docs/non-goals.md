...
> **host-local 例外（提案・2026-06-11 / Epic #6・ja#514、批准待ち）**: この非ゴールは **外部公開** を禁じるものであり、**ホストローカルに閉じた** MCP HTTP は対象外（例外）とする。`ORG_TRANSPORT=broker`（opt-in）で使う **`org-broker`** は `127.0.0.1` バインドの localhost-only HTTP MCP サーバーであり、外部到達不可・TLS / ネットワーク境界 / 外部認証の面を持たず、「ローカルで完結する運用規律」を崩さない。したがって broker は §12 の禁止対象（外部公開）ではなく、ローカル輸送層の一部として許容される。既定の `renga-peers`（ローカル標準入出力経由・同一タブ内 P2P）は不変で、本例外は broker を opt-in で足すだけの **加算的** な明確化（提案。批准は Epic #6 dogfood ゲート後）。契約面の詳細は [`docs/contracts/backend-interface-contract.md`](./contracts/backend-interface-contract.md) Surface 8（§8.6）。
...
- 2026-06-11: §12 に host-local MCP HTTP の例外注記を **提案（批准待ち）** として加算（Epic #6 / ja#514）。`org-broker`（localhost-only HTTP MCP、opt-in）は外部公開禁止の対象外であることを明確化。既定 `renga-peers` の記述・通信モデルは不変。
