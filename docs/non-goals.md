...
**代替手段**: 各ロールごとに `tools/role_configs_schema.json` を正典とした `settings.local.json` を `/org-setup` で配布します。許可エントリとフックはスキーマに登録され、CI (`tools/check_role_configs.py`) で設定の乖離を検出します。ディスパッチャーの `bypassPermissions` 例外そのものは、Sonnet で `auto` モードが利用可能になった時点で再評価します。
