...
フォアマンだけが `bypassPermissions` 固定の理由は **コスト最適化の必然的な帰結**です。Claude Code（TUI）では `auto` モードが Opus 系でのみ起動し、Sonnet では `auto` モード自体が利用できません。フォアマンを Sonnet で運用する以上、`permission_mode` の選択肢は実質 `bypassPermissions` 一択であり、これは分類器の挙動を回避しているわけではなく、そもそも `auto` を選べない環境での唯一の選択です。フォアマンのスコープはペイン起動・指示送信・状態記録に絞られているため、`git push --force` や任意のファイル編集のような事故クラスが直接発火する経路は限定的です。トレードオフは正直に開示します。

**代替手段**: 各ロールごとに `tools/role_configs_schema.json` を正典とした `settings.local.json` を `/org-setup` で配布します。許可エントリはスキーマに登録し、CI で設定の乖離を検出します。一時的に制約を緩める必要があれば、対象スコープを限定したフックや `permissions.allow` の追記をスキーマ経由で行ってください。フォアマンの `bypassPermissions` 例外を撤廃したい場合は、Sonnet で `auto` モードが利用可能になった時点で再評価します。
