...
**代替手段**: 各ロールごとに `claude-org-runtime` 提供の framework schema と ja-side `tools/org_extension_schema.json` をマージしたものを正典とし、`settings.local.json` を `/org-setup` で配布します。許可エントリとフックはスキーマに登録され、CI (`tools/check_role_configs.py`) で設定の乖離を検出します。ディスパッチャーの `bypassPermissions` 例外そのものは、Sonnet で `auto` モードが利用可能になった時点で再評価します。
