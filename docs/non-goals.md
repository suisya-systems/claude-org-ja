...
フォアマンだけが `bypassPermissions` 固定の理由は **コスト最適化と挙動安定の両立**です。フォアマンの主業務はワーカーペインの起動と指示送信であり、`auto` モードの安全分類器（safety classifier）は Sonnet では不安定でペイン起動が分類器にブロックされる事象が確認されています。Sonnet を維持しつつ作業を流すには分類器を迂回する必要があり、`bypassPermissions` をやむなく採用しています。フォアマンのスコープはペイン起動・指示送信・状態記録に絞られているため、`git push --force` や任意のファイル編集のような事故クラスが直接発火する経路は限定的です。トレードオフは正直に開示します。

**代替手段**: 各ロールごとに `tools/role_configs_schema.json` を正典とした `settings.local.json` を `/org-setup` で配布します。許可エントリはスキーマに登録し、CI で設定の乖離を検出します。一時的に制約を緩める必要があれば、対象スコープを限定したフックや `permissions.allow` の追記をスキーマ経由で行ってください。フォアマンの `bypassPermissions` 例外を撤廃したい場合は、Sonnet で `auto` モードが安定するか、または分類器に依存しないペイン起動経路が確立した時点で見直し可能です。
