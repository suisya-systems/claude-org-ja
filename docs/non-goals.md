...
| ディスパッチャー（Dispatcher） | `bypassPermissions` | Sonnet | **例外**。理由は下記 |
| キュレーター（Curator） | `auto` | Sonnet | 最小許可（読み取り中心 + ナレッジ整理に必要な書き込みのみ）。知見整理は軽量・機械的ワークロードのため Sonnet で十分 |
| ワーカー（Worker） | `auto` | Opus | 許可エントリの絞り込み + ロール別フック + タスクごとの作業ディレクトリ境界 |

ディスパッチャーだけが `bypassPermissions` 固定の理由は、`auto` モードの安全分類器が「子エージェント起動」（`spawn_claude_pane` によるワーカー派遣）を "Create Unsafe Agents" と判定してブロックし、`auto` ではワーカー派遣が成立しないためです。分類器はセッションモデルと独立した専用モデルで動作する（https://www.anthropic.com/engineering/claude-code-auto-mode）ため、これはモデル選択（Sonnet 運用）とは独立の制約であり、分類器の挙動を都合よく回避しているわけではなく、ディスパッチャーの職務（子エージェント起動）と `auto` が構造的に両立しないことによる唯一の選択です。

### `bypassPermissions` の実際の挙動（誤魔化さない）

`bypassPermissions` モードはその名の通り、`permissions.allow` と `permissions.deny` の **両方を bypass します**。つまり「`permissions.deny` に並べた `git push --force` 等の拒否ルールはディスパッチャー側では効きません」。Claude Code 公式が bypass モードでも残しているのは、`.git/`, `.claude/`, `.vscode/`, `.idea/`, `.husky/` 等の保護対象ディレクトリに対する書き込み確認プロンプトのみです。

したがってディスパッチャーの実効防御層は次のように整理されます:

1. **保護対象ディレクトリの自動確認プロンプト**（Claude Code 側で `bypassPermissions` でも保持される範囲のみ）
2. **PreToolUse フック**（`bypassPermissions` でも実行され、exit code 2 でツール呼び出しをブロックできる。allow ルールより優先される）。ディスパッチャー用には以下のフックが配備済みです:
...
3. **ロール契約による自主規律**（ディスパッチャーの `CLAUDE.md` に記載された業務スコープ宣言と、窓口によるライフサイクル監視）
...
**代替手段**: 各ロールごとに `claude-org-runtime` 提供の framework schema と ja-side `tools/org_extension_schema.json` をマージしたものを正典とし、`settings.local.json` を `/org-setup` で配布します。許可エントリとフックはスキーマに登録され、CI (`tools/check_role_configs.py`) で設定の乖離を検出します。ディスパッチャーの `bypassPermissions` 例外そのものは、`auto` モードの分類器が子エージェント起動（ワーカー派遣）をブロックしなくなった時点で再評価します。
...
**代替手段**: 現状のディスパッチャーは同時に複数のワーカーを派生できますが、ピークでも 3〜5 を上限の目安としてください。「大量の似たタスクをまとめて処理したい」用途なら、ワーカーを多重化するのではなく、タスクをまとめて 1 ワーカーに渡し、進捗をジャーナルで追う方が適しています。
...
**理由**: claude-org-ja は **Claude 専用**で立ち位置を取っています。複数プロバイダー対応は魅力的に見えますが、プロバイダーごとに許可モデル・フック機構・MCP サーバーとの互換性・文脈窓の形状・ツール呼び出し仕様が異なり、「規律を強制する」というフレームワークの本質がプロバイダー数だけ薄まります。Claude Code に深く張ることで、`renga-peers` MCP サーバー・フック・設定スキーマ・サンドボックス等、Claude Code 由来の規律を最大限活用できます（輸送層は両系。既定 renga の `renga-peers` も opt-in broker の `org-broker` もいずれも Claude Code ネイティブの輸送系であり、ここで言う「深く張る」恩恵は両系で共通。輸送系の二フレームは §12 host-local 例外を参照）。
...
**代替手段**: ツール許可の追加が必要なら、ja-specific entry については `tools/org_extension_schema.json` を更新してください（framework schema 自体は `claude-org-runtime` パッケージが SoT、ルール追加フロー: スキーマ → ドキュメント → 実 `settings.local.json` の順）。drift 検出時の対処手順は [docs/getting-started.md](getting-started.md) の `tools/check_role_configs.py` の節を参照。
...
**代替手段**: 共有が必要な情報は `knowledge/curated/` や `registry/` 等の窓口管理領域に置き、ディスパッチャー・窓口経由でのみ書き換えてください。
...
**理由**: claude-org-ja の MCP サーバーは既定運用フレームでは `renga-peers`（ローカル標準入出力経由）に集約されており、**同一タブ内 P2P** が通信モデルの正本です（opt-in `broker` の `org-broker` は `127.0.0.1` バインドの **localhost-only** HTTP MCP であり、これも「外部公開」ではない。下記 host-local 例外を参照）。**外部公開の** HTTP は認証・流量制御・TLS・ネットワーク境界という別レイヤの問題を呼び込み、「ローカルで完結する運用規律」というシンプルな保証が崩れます。
...
> **host-local 例外（2026-06-11 提案 → 2026-06-14 批准 / Epic #6・ja#514・dogfood #515 passed）**: この非ゴールは **外部公開** を禁じるものであり、**ホストローカルに閉じた** MCP HTTP は対象外（例外）とする。`ORG_TRANSPORT=broker`（opt-in）で使う **`org-broker`** は `127.0.0.1` バインドの localhost-only HTTP MCP サーバーであり、外部到達不可・TLS / ネットワーク境界 / 外部認証の面を持たず、「ローカルで完結する運用規律」を崩さない。したがって broker は §12 の禁止対象（外部公開）ではなく、ローカル輸送層の一部として許容される。既定の `renga-peers`（ローカル標準入出力経由・同一タブ内 P2P）は不変で、本例外は broker を opt-in で足すだけの **加算的** な明確化（2026-06-14 批准済み。Epic #6 broker dogfood #515 passed）。契約面の詳細は [`docs/contracts/backend-interface-contract.md`](./contracts/backend-interface-contract.md) Surface 8（§8.6）。
...
- 2026-06-11: §12 に host-local MCP HTTP の例外注記を **提案**（Epic #6 / ja#514）として加算 → **2026-06-14 批准**（Epic #6 broker dogfood #515 passed / PR #567）。`org-broker`（localhost-only HTTP MCP、opt-in）は外部公開禁止の対象外であることを明確化。既定 `renga-peers` の記述・通信モデルは不変。
