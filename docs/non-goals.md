...
ディスパッチャーだけが `bypassPermissions` 固定の理由は **コスト最適化の必然的な帰結**です。Claude Code（TUI）では `auto` モードが Opus 系でのみ起動し、Sonnet では `auto` モード自体が利用できません。ディスパッチャーを Sonnet で運用する以上、`permission_mode` の選択肢は実質 `bypassPermissions` 一択であり、これは分類器の挙動を回避しているわけではなく、そもそも `auto` を選べない環境での唯一の選択です。
