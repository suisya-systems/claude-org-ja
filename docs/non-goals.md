...
2. **PreToolUse フック**（`bypassPermissions` でも実行され、exit code 2 でツール呼び出しをブロックできる。allow ルールより優先される）。フォアマン用には以下のフックが配備済みです:
   - `block-dispatcher-out-of-scope.sh` — Edit/Write 対象を `.foreman/`, `.state/`, `knowledge/raw/YYYY-MM-DD-{topic}.md` に限定。アプリケーションコード（`tools/`, `dashboard/`, `tests/`, `.claude/skills/`, `docs/`, `registry/` 等）への直接編集は exit 2 でブロック
