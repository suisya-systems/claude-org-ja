...
> **スコープと不可触制約**: 本 runbook は「実走を可能にする手順書」であり、**本番 ja の broker 実走（org-start ハイジャック）は後日のトラック 3（ユーザー hands-on）で行う**。本書の手順はすべて **テスト用 state-dir（`.state/broker/` ではない別ディレクトリ）** で daemon を起動・停止し、本番 `.state/` を汚さない前提で書く。**既定 `renga` は削除せず opt-in fallback として常時有効**（切戻しの安全装置）。なおここで言う「既定 `renga`」は **運用フレーム**（broker 実走 dogfood が Epic #6 Issue G で活性化するまで運用上の既定経路が renga）を指す。これとは別の **コード定数フレーム**では `claude_org_runtime.transport.DEFAULT_TRANSPORT` が runtime 0.1.28（Epic #586）で `renga` → `broker` にフリップ済みで、両者は指す対象（運用経路 vs コード定数）が異なり矛盾しない（二フレーム注記 Refs #604）。
