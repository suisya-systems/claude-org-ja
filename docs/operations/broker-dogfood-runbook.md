...
> **スコープと不可触制約**: 本 runbook は「実走を可能にする手順書」であり、**本番 ja の broker 実走（org-start ハイジャック）は後日のトラック 3（ユーザー hands-on）で行う**。本書の手順はすべて **テスト用 state-dir（`.state/broker/` ではない別ディレクトリ）** で daemon を起動・停止し、本番 `.state/` を汚さない前提で書く。**既定 `renga` は削除せず opt-in fallback として常時有効**（切戻しの安全装置）。なおここで言う「既定 `renga`」は **運用フレーム**（上記**トラック 3**の本番 broker 実走が活性化するまで運用上の既定経路が renga。Issue G の #515 併存 dogfood 自体は 2026-06-14 に passed = broker を opt-in として批准済みであり、未了なのは本番昇格＝トラック 3 のみ）を指す。これとは別の **コード定数フレーム**では `claude_org_runtime.transport.DEFAULT_TRANSPORT` が runtime 0.1.28（Epic #586）で `renga` → `broker` にフリップ済みで、両者は指す対象（運用経路 vs コード定数）が異なり矛盾しない（二フレーム注記 Refs #604）。

> **検証ステータス**: 起動・停止・ライフサイクル・dry-run の各コマンドは 2026-06-11 に **runtime 0.1.17** / tmux 3.2a / WSL2 の worker worktree 環境で実機検証済み（生ログの要点は各節に埋め込む）。§8 の attach 導線は 2026-06-13 に **runtime 0.1.22** で確認。**本書の実機検証済み broker surface 記述は runtime 0.1.22 同期**（§1.1 setup / §2.1 serve flag に `--root-role` / `--root-cwd` / §3.6 journal / §3.8 admin RPC・sidecar / §5(4)-(5) sidecar disposal）。0.1.17 → 0.1.22 で増えた serve 面・admin 面・sidecar は `claude_org_runtime/broker/{cli,server,sidecar}.py`（0.1.22）から照合した（#515 dogfood の D2-D6 整理）。**push-first 追補（0.1.24+）**: §3.3 のナッジ配達記述には push 一次（per-pane channel sidecar / claude/channel、runtime push-first 0.1.24+、設計 SoT transport-lab `docs/design/broker-native-roles.md` §9）への位置づけ注記を含むが、上記実機検証は 0.1.22 surface 時点のものであり、**push 一次 surface 自体の実機検証は 0.1.24+ 環境で別途要する**（本追補は prose 整合のための位置づけであって 0.1.24+ の実走再検証ではない）。
...
# 停止（起動形態で場合分け）:
#   - 前景 serve（このシェルでブロック中）: Ctrl+C。前景プロセスへの SIGINT は
#     Python 既定ハンドラ経由で KeyboardInterrupt を上げ、graceful 停止経路 =
#     run() の finally で stop() が走り journal 末尾に broker_stopped が 1 行残り、
#     daemon.json / admin.token sidecar も削除される。
#   - detached daemon（nohup ... & 等で背景起動）: 正規経路は `org down`
#     （org ライフサイクル管理経由）か SIGTERM を送る:
#       kill -TERM <pid>
#     背景 daemon に SIGINT（kill -INT）は効かずプロセスが残存する。runtime の
#     signal handler は SIGTERM / SIGBREAK のみ配線で SIGINT を捕捉しないため、
#     背景プロセスは Ctrl+C を伴う端末を持たず KeyboardInterrupt も上がらない
#     （2026-06-13 切戻しドリルで 2 回再現。後日の実測でも kill -INT は 5 秒
#     無反応、続けて kill -TERM で即停止）。背景は SIGTERM か `org down` で止める。
...
§2.3 の serve は前景でブロックしているので、停止は **Ctrl+C**（前景プロセスへの SIGINT）で行う。前景 serve は Python 既定の SIGINT→KeyboardInterrupt で `run()` の finally を通り graceful に止まる（**clean shutdown なら exit code 0**）。**この経路が効くのは前景 serve に限る**: 背景 daemon には SIGINT は効かない（runtime の signal handler は SIGTERM / SIGBREAK のみ配線、§2.2）ので、background で回した場合は `kill -TERM` か `org down` で止める（実測: kill -INT 5 秒無反応 → kill -TERM 即停止）。
...
> **検証ログ（2026-06-11、実機）**: `--no-nudge` / `--backend tmux` の両系で `initialize → tools/list → send_message → check_messages` 往復が成功し、前景 serve への **Ctrl+C（SIGINT）で exit 0**（前景ゆえに KeyboardInterrupt 経由で graceful；背景 daemon には SIGINT は効かない、§2.2）。`tools/list` は worker tier で messaging 4 面のみ。本番 `.state/broker/` は未生成（テストパスにのみ `queue.jsonl`）。tmux backend は live tmux server なしでも adapter が遅延生成され、起動・停止が成立する（ナッジを実際に注入する子ペインが無いので messaging probe は skip）。
...
- ナッジ配達は **定型 1 行のみ PTY 経由**で注入し、本文は通さない（受信側は `check_messages` で pull 取得）。**push 一次への再設計後（runtime push-first、0.1.24+）は、この PTY ナッジ + pull は per-pane channel sidecar（`server:org-broker-channel` の `claude/channel` 注入）が効かない場合の*フォールバック*配送路**と位置づけられ、push 一次が runtime 既定の**配達モード**（broker 内部での push 対 pull の既定であり、transport の `renga`/`broker` 既定選択とは別軸。設計 SoT: transport-lab `docs/design/broker-native-roles.md` §9。本 runbook が同期している runtime surface 版での実挙動は当該版に従う）。adapter 不通や対象未着のときは `nudge_defer_interval`（既定 2.0s）× `nudge_defer_max_tries`（既定 30）まで再試行する。
...
> **対象範囲（重要）**: attach で見えるのは **broker が `adapter.spawn` した子ペイン（ディスパッチャー・ワーカー）** のみ。**窓口（root secretary）は adapter 実ペインを持たない logical pane**（bookkeeping entry。`register_logical_pane`、`claude_org_runtime/broker/server.py`）であり、org を起動した人間の手元 terminal でそのまま動く（broker socket には現れない）。したがって本導線が埋めるのは「ワーカー群 / ディスパッチャーの生画面が見えない」ギャップであって、窓口は元々人間の眼前にある。

### 8.1 現状 — 独立セッションへの attach（runtime terminal adapter）

現行 runtime の terminal adapter（tmux、`claude_org_runtime.terminal.tmux`）は、broker が spawn する子ペイン（ディスパッチャー・ワーカー）を**専用 socket `claude-org-broker` 上の独立 detached セッション**として作る（セッション名 `claude-org-broker-<pid>-<連番>`、`isolated_session = True`）。既存 tmux サーバー（renga 等）とは socket 分離されているため、観察には socket 名 `-L claude-org-broker` の明示が要る。

```bash
# 1) 現存する broker セッションを一覧（読み取りのみ。socket 明示が必須）
tmux -L claude-org-broker list-sessions
#   例:  claude-org-broker-12345-1: 1 windows (created ...)   ← 各行が 1 子ペイン（連番は 1 始まり）
...
>
> ※現行は claude-org-broker に改名（runtime 0.1.29 で破壊的変更）。
...
- spawn 儀式（folder-trust 承認 + dev-channel sidecar 承認の再導入。push 一次採用に伴う加算）: [`.dispatcher/references/spawn-flow.md`](../../.dispatcher/references/spawn-flow.md) 3-3b
