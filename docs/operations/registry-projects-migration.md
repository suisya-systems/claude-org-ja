...
上の表は `pull` で書いたが、実際の条件は「**HEAD が移行コミットを跨いで動くこと**」である。同じ削除は次でも起きる:

- `git rebase origin/main`（移行前のブランチを移行後の main の上に載せ直すとき）
- `git checkout <移行前のブランチ>` / `git switch` でのブランチ切替
- `git merge` で移行コミットを取り込むとき

本タスクの作業中、実際に `git rebase origin/main` でこの削除を踏んだ（clean ケースだったため退避なしで消えた）。**pull 前だけでなく、移行コミットを含むブランチへ移動する前は常に 2-1 の退避を先に取ること。** 復元は 2-3 と同じ。移行後のブランチ上に留まっている限りは何度 rebase しても再発しない（テンプレートから再生成すれば済む状態になる）。

## 2. 移行手順

### 2-1. 移行コミットを取り込む前に（全 checkout 共通・1 回だけ）

`pull` に限らず、移行コミットを跨いで HEAD を動かす操作（`rebase` / ブランチ切替 / `merge`）の前に、
