...
| **revoke（pane 退役）** | adapter の `pane_exited` イベント受領時、および broker 経由の `close_pane` 成功時に即時 revoke する。退役済み pane の token による呼出は `token_revoked` エラー（[§5](#5-contract-set-d-との整合差分表) の Surface 6）で拒否する。子プロセス等に env が漏洩していても、pane 退役後は使えない |
...
| 3. チャネル承認 | `send_keys(enter=true)` で「Load development channel?」プロンプトを承認 | **dev-channel prompt は存在しない**（dev-channel flag を使わないため）。`--mcp-config` 注入されたサーバーに対する Claude Code 側の信頼確認プロンプトの有無と機械承認可否は **AC-2（[§7.1](#71-phase-1-スパイク全ペイン-renga-free-起動の実証)）で実測し GO 済**（フォーク）。プロンプトが出る場合は現行同様 `send_keys(enter=true)` で機械承認できることを確認した |
...
dev-channel prompt の消滅により、現行 spawn-flow の 3-3b（Enter 承認）と「承認しないと list_peers 待ちがタイムアウトする」結合が解消される見込みである一方、新たに「MCP サーバー接続の信頼確認」という確認項目が入る。**段階 1・3・4 の置き換え成立（spawn → 接続 → 帰属 → 配達の一往復）は Phase 1 スパイクの合格条件 AC-2** であり、フォークで達成済（[§7.1](#71-phase-1-スパイク全ペイン-renga-free-起動の実証)）。3-3b / 3-4 相当の儀式を置き換えられないまま残る場合は Phase 2 以降に進まない、という gate を置いていたが、AC-2 GO により通過した。
...
> **tmux 第二 backend ステータス（フォーク実証）**: tmux を第二 backend として実装し、WezTerm 用 AC-1 / AC-2 ハーネスを backend パラメータ化（共有面を集約）。**POSIX（tmux 3.4 / WSL2）と Windows（WezTerm）の両 backend で AC-1（自動 3 状態）/ AC-2（接続チェーン）が green**。これにより tmux 列は「将来・参考」から **実装済み・実測値** に格上げした。下表の tmux 列はフォークでの実測に基づく。

#### 4.7.1 backend 横断の能力比較（全能力）

| 能力 | renga | WezTerm（`wezterm cli`） | tmux（`tmux`、フォーク実装） | 要求フェーズ |
|---|---|---|---|---|
| pane への send-text（ナッジ注入） | ○ `send_keys` | ○ `send-text`（既定 bracketed paste） | ○ `send-keys`（一級プリミティブ） | **Phase 3（messaging）** |
| 制御打鍵モデル（Enter / Ctrl-C 等） | ○（高レベル API） | △ `send-text --no-paste` + `CR` / ETX（paste 既定のため小細工要） | ○ `send-keys Enter` / `C-c` / `-l`（小細工不要。フォーク実測） | **Phase 3（messaging）** |
| grid scrape（静止確認 defer の状態判定） | ○ | ○ `get-text` | ○ `capture-pane -p`（フォーク実測） | **Phase 3（messaging）** |
| pane 識別の安定名管理 | ○（name/role をサーバー管理） | △（pane id のみ。name↔id 対応表を adapter が保持） | △（pane id `%N`。同左を adapter が保持） | **Phase 3（messaging）** |
| headless / GUI 非依存運用 | ○ | △（GUI 非必須。mux-server のみで spawn 可。フォーク実測） | ○（detached session で標準動作。display 不要。フォーク実測） | 運用前提 |
| split spawn（cwd / command 指定） | ○ | ○ `split-pane` | ○ `split-window` / `new-session`（spike は new-session で検証） | Phase 4 |
| geometry 付き list_panes（cell 単位 rect） | ○ | ○ `list --format json`（rows/cols/位置） | ○ `list-panes -F`（`#{pane_left/top/width/height}`。フォーク実測） | Phase 4 |
| grid scrape（`inspect_pane` 相当） | ○ | ○ `get-text` | ○ `capture-pane`（フォーク実測） | Phase 4 |
| cursor 位置付き scrape | ○（`include_cursor`） | △（`get-text` 単体では不可。別途取得・要検証） | ○（`list-panes -F` の `#{cursor_x/y}` を同一呼出で取得。フォーク実測。WezTerm より優位） | Phase 4 |
...
「adapter で何でも差し替え可能」とは主張しない代わりに、**どの面が Phase 3 で要り、どの面が Phase 4 で要るか**を 2 表に分離して固定する。フォークのスパイク（WezTerm + tmux）が実証したのは **messaging tier（+ 起動チェーン）** であり、full backend tier の配線替え（spawn / inspect / poll_events）は Phase 4 スコープで検証した（[§7.4](#74-phase-4-ペイン操作移行full-backend-adapter)）。

**(a) messaging adapter（Phase 3 が要求する最小面）** — WezTerm + tmux 両 backend で実証済み（フォーク）:
...
→ **結論**: messaging 移行に必要な面は **send-text + grid scrape + pane 識別 + 起動チェーン** に閉じ、renga / WezTerm / tmux いずれも充足。**メッセージング移行は backend 非依存**（WezTerm / tmux 両実証、フォーク）。
...
| geometry 付き list_panes | ○ | ○（フォークで形は実測） | 同等 |
...
→ **結論**: Phase 4 面は両 backend で概ね充足する。**`poll_events` のポーリング合成の実効遅延**（dispatcher 監視ループの正しさ）と **balanced split の現行同等性**は正準 backend = tmux でフォーク実証済み（[§7.4](#74-phase-4-ペイン操作移行full-backend-adapter)）。WezTerm は能力表上は充足見込みだが実機 AC は follow-up。
...
- WezTerm / tmux いずれかの常駐が新たな前提になる（依存の付け替え側面）。**フォークで tmux 第二実装を成立させたことにより「adapter 境界があるため二次移行は安価」という整理は机上ではなく実証になった**（同一ハーネス・同一 AC が backend パラメータ切替で両系 green）。POSIX 系は tmux、Windows 系は WezTerm を正準 backend とする運用が可能。

## 5. Contract Set D との整合（差分表）

[`docs/contracts/backend-interface-contract.md`](../contracts/backend-interface-contract.md)（2026-05-03 批准）に対する本設計の位置付けを Surface 単位で固定する。**本節は「改訂提案」であり、批准済み契約の本文は変更しない**。改訂の実施は、フォーク実証を経て本体取り込み（フェーズ取り込み）と同時に正式な契約改訂 PR（Set D の amendment 手続き）として別途行う計画である。二重正本を作らないため、**改訂が批准されるまでの正本は現行 Set D 本文**である。
...
> **Phase 4 ステータス（フォーク実証）**: spawn / close / list_panes / inspect_pane / send_keys / poll_events を broker + adapter へ配線替えし、下記完了基準を **全項目 GO** で実証した。実機 backend は本環境（Linux/WSL2）で WezTerm 実機が使えないため正準 backend の tmux に読み替え（窓口経由の人間判断で承認。tmux 実機 AC の前例〔messaging tier 検証時〕に沿う）。WezTerm 実機 AC は follow-up。
...
> **完動ゲートの定義（2026-06-11 再定義）**: 新前提（[§1.2](#1-背景と確定制約) 制約撤回）に合わせ、Plan B（renga 依存解消）の完動ゲートを次のとおり定義する: **全ペイン（窓口を含む）が tmux または WezTerm の pure backend で renga-free に org-start でき、委譲サイクルを完走する**こと。旧定義（「輸送層だけ renga 不使用・人間入力の窓口は renga 継続」）の二重構造前提は撤回した。この再定義は、(i) 既に GO 済の **renga-free 輸送 dogfood**（broker + tmux adapter で委譲サイクルを複数回完走。正準 backend = tmux、WezTerm 実機は follow-up）と、(ii) **IME 非阻害の経験的確定**（窓口の人間入力を pure backend で行ってもスピナー描画・ナッジ注入が日本語 IME を壊さない。[§1.2](#1-背景と確定制約)、2026-06-11 + 2026-06-08）の合成で満たされる — IME こそが「窓口だけ renga に縛る」最後の根拠だったため、その撤回で全ペイン renga-free が完動ゲートの正式な合格条件になった。

**完動ゲート = GO（フォーク claude-org-transport-lab で実証）**。完了基準 4 項目をすべて GO で満たした:

- **(1) backend（tmux）のみ・renga 不使用で全ペイン起動（org-start 相当）+ 委譲サイクルを複数回完走** → 単一 broker / adapter 上で 3 サイクル連続完走 + cross-cycle isolation（native id 再利用下でも旧 handle は `pane_not_found`、inbox / token / event cursor がサイクル間で非漏洩、二重 spawn は名前衝突で拒否）。実機は tmux cat 2 サイクル + 実 Claude worker active 1 サイクルを含む。
...
| WezTerm / tmux 常駐の新前提化 | renga 依存を外す代わりに端末 backend（WezTerm / tmux）+ broker デーモンが前提に加わる。adapter 境界により二次移行は安価 — **フォークで tmux 第二実装を成立させ、同一 AC が backend パラメータ切替で両系 green になることを実証**（[§4.7](#47-terminal-adapter-の境界と能力表)）。POSIX=tmux / Windows=WezTerm の使い分けが可能 |
...
- **tmux adapter**: フォークに実装済み（POSIX 正準 backend）。能力表（[§4.7](#47-terminal-adapter-の境界と能力表)）の tmux 列は実測値に更新済み。残るスコープ外は messaging / pane-control 以外の tmux 固有機能（copy-mode 連携等）であり、本設計の対象外。
...
- 2026-06-11: 設計再導出を反映（design only）。確定制約 #2（IME 制約により WezTerm 素は不成立・人間入力端末は renga 継続）を経験的根拠（描画層の backend parity 検証 2026-06-11 + 輸送層の broker ナッジ × IME 変換中の手動 AC 2026-06-08）で**撤回**し、採用方針を「輸送層だけ renga 非依存・窓口は renga 継続」の二重構造から「**全ペイン（窓口を含む）が tmux / WezTerm の pure backend で renga-free に完全移行、renga は opt-in fallback として任意残置（廃止しない・破壊最小・切戻し可）**」へ改訂。Phase 1 の足切り（計画中止）条項を撤回し、完動ゲートを「**全ペイン renga-free で org-start + 委譲サイクル完走**」へ再定義（[§7.6](#76-完動ゲート全ペイン-renga-free-で-org-start-と委譲完走)）。能力表（§4.7）の tmux 列を実測値へ格上げし、messaging / full backend の能力境界を 2 表に分離。実証（spike の Phase 1〜4 + 完動ゲート）はフォーク claude-org-transport-lab で完了済み（本リポジトリは design only。実装・runtime 挙動・他ファイル・GitHub には触れない）。
