...
> 一次入力: ユーザー・窓口間の設計合意ノート（2026-06-07、リポジトリ未コミットの運用ノート `notes/renga-decoupling-design-input-2026-06-07.md`）および Codex design review（同日、`tmp/` 配下の未コミットノート）。本設計はこれらの確定制約を覆さない。
...
| 4. 登録待ち | `list_peers` に worker が現れるまで 2 秒間隔リトライ（最大 30 秒） | **token 初回使用 = 登録**に置き換える。worker 側 Claude の MCP クライアントが broker に接続（initialize handshake）した時点で broker が bind 表を「登録済み」に遷移させ、`agent_ready` イベントを emit する計画。dispatcher は `poll_events`（または `list_peers` 相当）で同じ待ち方ができる |
...
dev-channel prompt の消滅により、現行 spawn-flow の 3-3b（Enter 承認）と「承認しないと list_peers 待ちがタイムアウトする」結合が解消される見込みである一方、新たに「MCP サーバー接続の信頼確認」という未知数が入る。**段階 3 と 4 の置き換え可否は Phase 1 スパイクの検証項目**に含める（[§7.1](#71-phase-1-スパイクwezterm--windows中止判断点)）。
...
2. **spawn 直後の儀式の変化**: dev-channel 承認（spawn-flow 3-3b）と `list_peers` 登録待ち（3-4）が、信頼確認プロンプト（有無は Phase 1 で実測）と `agent_ready` 待ちに変わる。
...
**合格条件（AC）— ナッジ注入の 4 状態テスト（go/no-go）**:
...
- 判定は**全 4 状態の合格が必須**。1 つでも「窓口入力を壊す」結果が再現可能に出た場合、緩和策（[§4.3](#43-窓口への割り込み配達ナッジ最難関足切り対象) の静止確認 defer 等）を尽くしても解消しなければ、**本計画ごと中止（棚上げ）する**。足切り条項であり、Phase 2 以降に進まない。
- 付随する実測確認項目（合否はナッジ AC のみで判定するが、設計の前提検証として記録する）: `--mcp-config` 注入時の信頼確認プロンプトの有無（[§4.6](#46-起動フローの置き換え) 段階 3）、token env 受け渡しの成立、`agent_ready` 相当の登録検知の成立、Windows（PowerShell / ConPTY）での send-text の文字化け有無。
...
- 取り込み時の同時変更: 分類 (b) の許可スキーマ再宣言、分類 (a) のメッセージング系 prose 書き換え、Set D Surface 2 / 5 の契約改訂 PR、non-goals §12 の改訂提案。
...
| リスク | 整理 |
|---|---|
| ナッジ注入の混線 | 受信側が長文入力中だと renga のチャネル注入より一段劣る。静止確認 defer（[§4.3](#43-窓口への割り込み配達ナッジ最難関足切り対象)）で緩和するが、最終判定は Phase 1 の 4 状態 AC（[§7.1](#71-phase-1-スパイクwezterm--windows中止判断点)）。**壊れたら計画ごと中止** |
