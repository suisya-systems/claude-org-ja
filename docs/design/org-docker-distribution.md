...
| A1 | Pi 5 実機（16KB / 4KB 両カーネル）での起動 | **未実施**（A2 完了により、以下が実機でしか測れない残件として確定した）。**(1) sandbox canary の初判定**: A2-q のとおり QEMU では EINVAL で測れないため、Pi 5 が S1 / S2 / S5 / I3 相当を arm64 で判定する最初の環境になる。最初の確認点は `docker exec <container> org-sandbox-canary` が `pass` を返すか。`enforce` のままだと canary 失敗＝コンテナ起動失敗として現れるので、切り分け時は `ORG_SANDBOX_CANARY=warn` で起動して canary を単体実行すると原因を分離しやすい（**単体実行は `docker exec -e ORG_SANDBOX_CANARY=enforce` で戻すこと** — `docker exec` はコンテナの env=warn を継承し、warn の canary は失敗しても exit 0 を返す。[`docker/sandbox-canary.sh`](../../docker/sandbox-canary.sh):68-73）。**(2) 16KB page kernel と Rust バイナリ**（§11）: 該当は herdr（aarch64 static-pie）と Claude Code 同梱 ripgrep。ripgrep は独立ファイルではなく Claude Code 単体バイナリに内蔵され実行時に展開されるため**静的検査では判定できず、実機で実際に走らせて初めて判る**。クラッシュ時の切り分けは `kernel=kernel8.img`（4KB）へ切替。**(3) 検証経路はビルドでなく pull**: A2 はローカル QEMU ビルドなので、実機では registry から arm64 image を pull して起動する経路（§11 の `--push` 前提のマルチアーキ manifest）を測るのが本筋。**pull 対象は P1 で publish 済み**（`ghcr.io/suisya-systems/claude-org-ja:v1.1.0-97-g6603478-r0.1.42-arm64`、private）。実機で叩くコマンド列は [`docker/README.md`](../../docker/README.md) の「公開済み image を pull して使う（Raspberry Pi 5 導線）」。**なお §11 正路である 2 プラットフォーム同時ビルド（`--platform linux/amd64,linux/arm64` + manifest merge）は A2 のスコープ外で未実測** — docker driver では `--load` できず `docker-container` driver + `--push` が要るため |
