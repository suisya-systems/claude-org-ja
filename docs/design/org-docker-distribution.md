...
実測結果は §12 の S0–S6 行（2026-08-14）。要点は 2 つ: (a) **出荷 image に `bubblewrap` が同梱されていない**ため、現状の compose 既定は「内側 sandbox を生かすために外側 seccomp を緩める」という本節のトレードオフの対価だけを払って便益を得ていない（S0）。(b) bwrap が**在るが機能しない**構成（S1 相当）では Claude Code が警告を一切出さず、`sandbox.failIfUnavailable` でも捕まらない（S6-d）。検出できるのは `claude-org-runtime sandbox doctor` の live canary のみ。
...
| S0 | 出荷 image の sandbox 依存充足（S1–S6 の前提） | **実施済み・不成立**（2026-08-14 実測。現行 [`docker/Dockerfile`](../../docker/Dockerfile) の apt 一覧に `bubblewrap` が無く、**出荷 image に bwrap が同梱されていない**。Claude Code の sandbox 依存 3 点のうち `socat` は同梱・`ripgrep` は Claude Code 同梱で充足するが `bwrap` のみ欠落。したがって出荷構成では S2/S5 で緩めた `seccomp=unconfined` の対価に見合う内側 sandbox が存在しない。S1/S2/S5 の実測は、この欠落を埋めた派生 image（出荷 image + `apt-get install bubblewrap`）で「設計が意図したマトリクス」として測定した。**対応方針は 3 点とも採用の判断（2026-08-14）**: (1) Dockerfile に `bubblewrap` を同梱する、(2) コンテナ既定を `sandbox.failIfUnavailable: true` にする、(3) 起動経路に `sandbox doctor` の実起動テストを組み込む。**実装は別タスク**（本タスクは実測と記録のみでコードを変更していない）） |
