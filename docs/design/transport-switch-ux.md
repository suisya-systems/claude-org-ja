...
- 非破壊不変条件は**この層から自動的に従う**: 会話なし → env 未設定 → 既定 renga → `rewrite_allow_entries` 恒等 → bit 等価（[§5.5](#55-非破壊不変条件の因果連鎖)）。
...
| **`org-start`** | 起動報告で現在の輸送系を常時 1 行可視化する表示主体（[§6](#6-機構-2-org-start-起動報告への現在輸送系の常時-1-行可視化)）。起動時に確定している輸送系（[§5.4](#54-提案する持続選択機構persisted-choice) の persisted choice）を `resolve()` 経由で読み、報告に添える |

> **design only の注記**: 上記は規範文書（`CLAUDE.md` / 各 SKILL）への**変更提案ではなく**、会話トリガ・可視化を「どの役割がどこで担うか」の設計上の位置付けである。実際の prose 反映は別スコープ（本設計の取り込み判断を経た後）であり、本設計書は規範文書に触れない。
...
- **broker 選択は opt-in / 切戻し可の判断**として扱う（[`docs/design/renga-decoupling.md`](./renga-decoupling.md) §1 採用方針＝完全移行・renga は opt-in fallback に整合）。窓口は選択を代行設定するだけで、broker daemon の起動・dogfood 実行（トラック 3）には踏み込まない（[§8](#8-issue-g-トラック-3-との独立性)）。
...
- **解決順は不変**: 永続選択 → env への反映は「env 層」への供給に閉じる。明示引数（最上位）は依然それを上書きでき、永続選択が無ければ env 未設定 = 既定 renga に落ちる。`resolve()` の優先順位ロジック自体には一切触れない。
...
> **代替案と却下理由（1 行）**: 「窓口プロセスの env を変えて spawn 継承に賭ける」案は、renga の spawn が窓口 Claude のセッション途中 env を継承する保証が無い（[§5.3](#53-伝播チェーン窓口--ディスパッチャー--ワーカー)）ため、頑健性で persisted choice に劣る。「永続化せず毎回会話で指定」案は、ディスパッチャー / ワーカーが独立に生成器を走らせる (A) 経路で選択を共有できないため却下。

> **design only**: 上記は機構の**提案**であり、永続点の新設・起動シーケンスへの組み込みは実装スコープ（規範文書・state schema は本設計書では変更しない）。

### 5.5 非破壊不変条件の因果連鎖

[§2](#2-不可侵の前提制約の明文化)-1 が会話 IF を足しても保たれることの**証明**（因果連鎖）:
...
すなわち、会話 IF は**既定 renga 経路に対して完全に passive**である（呼ばれなければ何も起きない）。これが「会話 IF を被せても非破壊不変条件は不可侵」の構造的根拠である。
...
|---|---|---|
| broker を 1 コマンドに前置 | `ORG_TRANSPORT=broker python3 ...` | `$env:ORG_TRANSPORT = "broker"; python ...`（前置形が無いので set → 実行 → 後始末） |
| broker を当該セッションに設定 | `export ORG_TRANSPORT=broker` | `$env:ORG_TRANSPORT = "broker"` |
