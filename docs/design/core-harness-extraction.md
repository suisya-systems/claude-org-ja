...
| `worker_roles{}` schema | worker template の **構造定義** (`permissions{allow,deny}`, `hooks{PreToolUse[]}`, `env{}`, placeholder `{worker_dir}` / `{claude_org_path}` / `{core_harness_path}`) | 個別 template 名 (`default`, `claude-org-self-edit`, `doc-audit`) は持たない |
...
- **対象**:
  ...
  - placeholder に `{core_harness_path}` を追加（inventory §5.2-6 への対応）
- **ja 側変更**:
  - `tools/role_configs_schema.json` を **org-specific entries 専用** に縮退（concrete role 名 + `secretary.required_allow` 38 entry + `^mcp__claude-peers__` ban + `worker_roles{default,claude-org-self-edit,doc-audit}`）。先頭で `"$framework_schema_ref": "core-harness>=0.1,<1.0"` を宣言
...
- **対象**:
  - `pyproject.toml` (ja に存在しない場合は新規) に `dependencies = ["core-harness>=0.1,<1.0"]` を追加
  - もしくは `requirements/core-harness.txt` で `core-harness @ git+https://github.com/suisya-systems/core-harness@v0.1.0` を pin
