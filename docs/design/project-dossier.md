...
### 4.2.1 プロファイル slug と実効 project_slug の不一致

`--profile` はプロファイル層に `project_slug` を置くが、より強い層（`--from-toml` の `[project].name` / `--project-slug`）がこれを上書きできる。上書きされても**プロファイル由来の他の値**（埋め込まれた charter / notes、説明、commit prefix、ブランチ様式）は台帳のプロジェクトのものが残る。

この混合は「プロジェクト X を担当すると告げられたワーカーの brief に、プロジェクト Y の憲章と commit 規約が入る」という状態であり、brief は台帳の出所を名乗らないため人間が気づける保証もない。**したがってエラーとする。** 兄弟の失敗（台帳不在・未定義クラス・禁止キー）が全て loud である以上、identity 不一致だけ silent にする理由がない。
...
- **C** 書き込み主体を分離する。org-curate は**振り分け判定と note 草稿の提案**までを行い、`registry/projects/<slug>/notes/` への実書き込みは `registry/` を所有する**窓口**が行う。

**採用: C**（2026-07-20、人間承認）。批准済み契約を一切改訂せずに矛盾が消え、可逆性が最も高い。将来キュレーター直書きの必要性が実測で出た時点で A へ格上げを提案する余地も残る。

**受け渡しの具体化（必須）**: 「curator は書かない」だけでは、curator が `registry/` 内に下書きを置いたり未認可 surface に一時ファイルを作ることで分離が形骸化する。したがって受け渡し成果物を次の 2 形態に限定し、それ以外を明示的に禁止する:

1. **peer message** による提案（振り分け判定 + ノート草稿本文）
2. **許可された surface 上の成果物**（`knowledge/curated/` 配下、またはスキル候補キュー）

`registry/` 配下への書き込みは**下書きであっても禁止**（「下書きなら許される」は抜け穴になる）。窓口はこれを受け、**provenance**（出所パス）と **privacy review**（オペレーター私的情報の混入有無）を付したうえで台帳へ適用する。適用可否の判断も窓口が持つ。実装は [`.claude/skills/org-curate/SKILL.md`](../../.claude/skills/org-curate/SKILL.md) Step 3.5。

**振り分け基準**（何がプロジェクト固有ノートで、何がグローバル curated 知識か）:
...
> **配置済み**（`registry/projects/claude-org-en/`）。配置にあたっては 2 段の障害があった。(1) `.hooks/block-org-structure.sh` は out-of-org ワーカーによる `registry/` 配下の**新規作成**を拒否する（Issue #736 の緩和は既 tracked ファイルの編集のみ）ため、`registry/` を所有する窓口が空ファイルを `git add` して tracked 化し、ワーカーが中身を記入する手順を取った。git は空ディレクトリを追跡しないため**編集予定の全ファイル**を実ファイルで用意する必要がある。(2) 実行される PreToolUse フックは `CLAUDE_ORG_PATH` 側の実体であり、その作業コピーが main より遅れていると main にマージ済みの #736 例外が実行系に存在しない（実測: 旧版 188 行 / #736 入り 208 行）。詳細は `registry/projects/claude-org-en/notes/hook-false-fire-on-registry-edit.md`。
...
- `.hooks/block-org-structure.sh` は out-of-org ワーカーの `registry/` 配下**新規作成**を拒否するため、ワーカーが台帳を新規に作れない（Issue #736 の緩和は既 tracked ファイルの編集のみ）。本タスクは「窓口が空ファイルを tracked 化 → ワーカーが記入」で回避したが、新規プロジェクトの台帳を立てるたびにこの二人三脚が要る。#736 の続きとして、対象 repo が `registry/` を正当に所有する場合は新規作成も許可する拡張を別 Issue で検討する価値がある。
- `contracts/` の参照のみ規約は**ファイル名ベース**の警告であり、`README.md` という名前で契約本文を置けば検出できない。第 1 段では規約 + 観測可能性までとし、内容検査は行わない（本文の意味判定は機械では担保しきれず、`contracts/` は実行系が一切読まないため実害経路が無い）。
