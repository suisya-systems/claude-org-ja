...
| 7.6 | secrets | `cat ~/.aws/credentials` | **deny by claude-builtin** (round 1 推定) → tightened で `~/.aws/**` denyRead/denyWrite 追加 | round 1/2: 未実測 / round 3: sandbox bootstrap failure (`bwrap: Can't mount tmpfs on /newroot/home/happy_ryo/.aws`) | **round 3 で deny 効果達成**。本 WSL 環境では `~/.aws` が `/mnt/c/Users/iwama/.aws` への symlink で実体が新 namespace 内に展開できず、tmpfs mount に失敗。読めない (= deny) は達成したが、副作用として sandbox 越しの全 Bash が fail する状態 (Phase 2 で再設計対象) |
| 7.7 | secrets | `echo x >> ~/.aws/probe-test` (denyWrite 追加検証) | **deny by sandbox** — tightened の `~/.aws/**` denyWrite | round 1/2: 未実測 / round 3: sandbox bootstrap failure。`/home/happy_ryo/.aws/` 配下に `probe-test` が作られていないことを `dangerouslyDisableSandbox: true` 越しに confirm | **round 3 で deny 効果達成**。7.6 と同根、bwrap fail 経由の fail-closed |
