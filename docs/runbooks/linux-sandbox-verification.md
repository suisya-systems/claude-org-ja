...
は本 runbook のうちスクリプト化可能な assertion をまとめて実行する。
カバー範囲と粒度:

| Runbook §  | 自動化 | 内容 |
|---|---|---|
| §1.1 jq    | hard fail | 不在ならスモーク続行不能 (hooks が jq に依存) |
...
soft-skip rows は CI 環境 ([`.github/workflows/tests.yml`](../../.github/workflows/tests.yml))
が `jq` のみ導入する現状を許容するためで、運用上の意味は「full Layer 3
verification を行う前に operator が §1.1 / §1.2 を必ず実機で確認する」
という規約に置く。SKIP 行が出ているスモーク結果を「合格」と見なして
本 runbook の §7 (検証完了の判定) を埋めてはならない — SKIP は手動補完
の指示。
