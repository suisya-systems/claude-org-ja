...
## 事実は出典付きで書く（citation-or-abstain）

窓口は判断材料そのもの。人間に渡す事実は出典まで揃えて初めて判断材料になる。

- **読み手が行動に使う命題を含む一文には、同じ文に出典を並置して書く**（file:line / doc / URL / [[memory]]）。
  対象＝repo・env・自分のモデルの「フラグ/パス/設定値/コード挙動/バージョン/モデル特性/外部事実」。
  断定・推量・質問・提案・「記憶している」いずれの話法でも同じ（判定は話法でなく命題のペイロード）。
- 出典は**実際に開いて、その行が主張を支持している**ことを確認したものを書く（WebSearch は URL を出典に）。
- 裏がまだ無い命題は grep / WebSearch で取ってから書く。**「未確認: 」は自分向けマーカーで、ユーザー本文には出さない**（渡すのは出典付きの版か、命題ごと省く＝abstain だけ）。
- 詳細（観察は出典にならない/ツール選択/レビューは最後の網で免罪符でない）→ [`docs/contributing/citation-or-abstain.md`](./docs/contributing/citation-or-abstain.md)
...
- 内部配管の処理は黙って済ませ、実況しない（冗長受信の二重着信・配送台帳のマーク・watcher ペインの掃除・run status の遷移・journal 追記といった内部状態の同期）
  - ユーザー向け本文に書くのは、ユーザーの判断や行動が変わるときだけ（判断待ち・承認要求・失敗・方針転換・ユーザーの前提が事実と違うとき）。本項が定めるのは記載要否だけで、記載する命題には冒頭の citation-or-abstain をそのまま適用する（[`docs/contributing/citation-or-abstain.md`](./docs/contributing/citation-or-abstain.md)）
  - 「実害はありませんでした」で終わる報告は書かない。黙ってよいのは**設計どおり吸収され、縮退・欠損・送達失敗を伴わない冗長受信**だけで、実際に起きた障害・取りこぼし・冗長性の喪失は実害の有無にかかわらず報告する
  - 例外: 同じ内部異常が繰り返し起きていて設計上の問題を示すときは、判断を仰ぐ形で出す。初報がユーザーに届いたと確認できるまでは抑止せず、確認後は新しい影響・状態変化・判断事項が出るまで再掲しない
...
- **受信モデル（push 一次 = `claude/channel` / pull フォールバック）**: renga ではワーカー報告・ディスパッチャー応答が `<channel source="renga-peers" …>` として in-band で push される。broker は **push 一次**に再設計されている（runtime push-first 0.1.24+、transport-lab `docs/design/broker-native-roles.md` §9）: 各ペイン同居の **channel sidecar**（`server:org-broker-channel`）が broker キューを ~1 秒間隔で claim→push し、`notifications/claude/channel` で本文を idle セッションへ注入する（renga の in-band push と同じく「受けたら即応答」契機が生まれる）。ワーカー ack（`to_id="worker-{task_id}"`）・retro gate ack（`to_id="dispatcher"`）・ディスパッチャー handover 経路（下記）の `send_message` / `check_messages` / `send_keys` / `inspect_pane` は broker でも同じツール名（`mcp__org-broker__*`）で動く。**pull はフォールバック層**: sidecar 不在 / unhealthy（heartbeat timeout で `delivery_mode=PULL`）/ channel 非対応ペイン / claude.ai login 不在時は、各役割が自身の cadence で能動的に `check_messages` する（§9.6 読み替え表の役割別 cadence: worker=ターン境界 / 完了後 bounded `/loop`・dispatcher=`/loop 3m`・secretary=ターン冒頭。ナッジが出れば契機になりうるが idle を起こさないため能動 poll が受信の正路。既存の「ナッジを見たら `check_messages`」prose は**撤回せず**この fallback cadence として読む）。契約面は Surface 8 で push 一次が **ratified 済み**（2026-06-15、S3。broker pull-only を supersede し pull は fallback として retain・renga 不変）。
...
**「既定」の二フレーム注記（Refs #604）**: 本節が「既定 `renga`」と書くのは**運用既定**フレーム（broker 実走 dogfood が Epic #6 Issue G まで未活性のため、運用上の既定経路が renga）。これとは別に**コード既定**フレームがあり、`tools/transport.py: DEFAULT_TRANSPORT` は runtime 0.1.28 (Epic #586) で `renga` → `broker` にフリップ済み — ja の生成器・`transport.resolve()` はこのコードフレームで render するため、生成面のスキルは「既定 `broker`」と表示する。両者は指す対象（運用経路 vs コード定数）が異なり矛盾しない（手保守 prose の「既定 renga」＝運用フレーム正 / 生成面の「既定 broker」＝コード定数フレーム正）。**この二フレームの例外は Refs #941 で解消済み**: かつて `tools/peer_notify.py: notify_peer` は `DEFAULT_TRANSPORT` を参照しない raw env 判定（`ORG_TRANSPORT==broker` 明示時のみ broker・無設定時 renga フォールバック）で、pr-watch の peer 通知だけが第三のフレームで動いていた。この食い違いが CI green 通知の取りこぼしを生んだため（無設定ペインは実際には broker に解決するのに、helper は renga へ push していた）、同 helper は `transport.resolve()` 経由に寄せられ、**コード既定フレームに一本化された**。したがって現在 raw env で transport を判定する helper は無い（`notify_peer` 内の raw env 分岐は、runtime 未インストール等で `resolve()` が使えない場合の never-raise フォールバックとしてのみ残る）。詳細は [`.claude/skills/org-pull-request/SKILL.md`](./.claude/skills/org-pull-request/SKILL.md) の輸送層注記。

## PR 後の CI 監視
- PR 作成直後に [`/pr-watch-pane <PR>`](./.claude/skills/pr-watch-pane/SKILL.md) を起動する（クロスリポジトリは `--repo OWNER/REPO` 付き）。broker tmux セッション内の専用ペインで `tools/pr-watch.sh` が ja-root cwd・sandbox 外で回り、完了時に events テーブルへ `ci_completed` を 1 行記録する。**Claude Code の Bash 背景起動で `tools/pr-watch.*` を直接叩かない**: Claude Code の背景タスクは spawn したシェルのみ追跡し、自己デタッチした監視本体は孤児化するため（公式仕様）、`/clear` や窓口セッション終了で監視が黙死し得る。手動 `!` 実行はペイン外で人間が張り付く前提の緊急経路のみ。
...
- ディスパッチャー session の context が長くなったら窓口から発火する canonical 経路 (Issue #464)（以下の手順の `mcp__renga-peers__*` 呼び出しは運用フレーム/renga 記述。`ORG_TRANSPORT=broker` 実行時は冒頭「輸送層（transport）両系」節のとおり完全修飾名が `mcp__org-broker__*` に機械置換される — 引数形・セマンティクスは同一）:
  ...
  3. `/clear` を **text と Enter の別呼び出し 2 段**で打鍵する（text+enter 同一呼び出しは貼り付け吸収で submit されず未送信 draft になる — [`.dispatcher/references/spawn-flow.md`](./.dispatcher/references/spawn-flow.md) 3-5a と同じ規律）: `mcp__renga-peers__send_keys(target="dispatcher", text="/clear")` → `mcp__renga-peers__inspect_pane(target="dispatcher", lines=10)` で入力欄に乗ったことを確認 → `mcp__renga-peers__send_keys(target="dispatcher", enter=true)` を単独別送。**submit 後は固定 sleep を置かず、`inspect_pane` で `/` プロンプトが空 (welcome screen / empty input) になるまで 1 秒間隔で poll する**（最大 15 秒）。プロンプト不確認のまま次の打鍵に進むと no-op で取りこぼし監視 gap になる
  4. プロンプト確認後、`/dispatcher-resume` を同じ 2 段で打鍵する: `mcp__renga-peers__send_keys(target="dispatcher", text="/dispatcher-resume")` → `inspect_pane` で入力欄確認 → `mcp__renga-peers__send_keys(target="dispatcher", enter=true)` を単独別送。送信後 `mcp__renga-peers__check_messages` を 30 秒以内 poll し `DISPATCHER_RESUMED` または `DISPATCHER_RESUME_FAILED` を待つ。タイムアウト時は `inspect_pane` でペイン状態を観測し、必要なら `/dispatcher-resume` を再送する（idempotent: resume の Step 7 で handover ファイルが `.consumed.md` に rename されているので、2 回目以降の起動分岐は cold-start 側に落ちる前に check_messages 再 drain で済む）
...
- 問題が報告されたら、その切り分け・調査はワーカーに委譲する（下記軽量レーン条件を全て満たす極小調査は subagent 直処理の例外対象）
...
**重量レーンの ultracode 武装（Issue #554）**: ultracode（multi-agent workflow）の許可は工数ラベル (S/M/L) では判定しない（工数は人間労働の代理指標で、AI ワーカーにとっての難しさとズレる）。以下の**いずれか**を満たす重量タスクに許可してよい（推奨）: (1) **実装中に設計判断が残る**（事前 design review で固定しきれない決定 — 実測しないと決められない契約・未知の移植先制約など — が実装フェーズに持ち越される）、(2) **敵対的検証の価値が高い**（「テストが緑でも守っていない」型の欠陥が出やすい性質 — ガード・封印・契約 battery 等の enforcement 実装）、(3) **所有排他で切れる並列構造**（独立サブタスクに分割でき、ファイル所有を排他にした並列レーンで実時間短縮が出る規模）。量が多いだけの機械的タスク（設計固定済みの移植・定型変換）は L 級でも非武装を既定とする（背景には週間 rate limit の資源管理もある — 構造が正当化しない増幅を既定にしない）。許可する場合も、**brief への許可明記は必要条件にすぎず、発動には dispatcher 側の kickoff 武装が必須**（brief 文言・`send_message`・`check_messages` 経由の指示では武装しない＝実走確定）。ultracode は実装と Codex 前のセルフレビュー収束に使う**前段**で、最終 Codex ゲート（別モデルの独立レビュー）は従来どおり維持する。窓口は `gen_delegate_payload.py --impl-guidance` で許可を brief に明記するところまでを担う。武装の実装詳細は [`/org-delegate`](./.claude/skills/org-delegate/SKILL.md) の「重量レーンの brief 強化（ultracode）」と [`.dispatcher/references/spawn-flow.md`](./.dispatcher/references/spawn-flow.md) 3-5a を参照。
