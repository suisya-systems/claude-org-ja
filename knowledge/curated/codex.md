...
```bash
# --base には origin/main を渡す（worktree 運用で local main が stale なことがあり、
# 古い base に対する diff だと既存マージ済みコミットまでレビュー対象に膨らむため）。
# 共有ブランチで他 worker の先行コミットが既にある場合は自分の直前コミットを渡す:
#   codex exec review --base <prev-commit> -m gpt-5.5 -c model_reasoning_effort=medium < /dev/null
# review surface は高速なので前景実行し、出力（Blocker/Major 相当）を見てから次に進む。
# stdin は < /dev/null で閉じる（背景化時の stdin 待ちハング回避）。
