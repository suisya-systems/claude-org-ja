...
claude-org は Claude Code をベースにした自己組織化マルチエージェントフレームワークです。窓口・ディスパッチャー・キュレーター・ワーカーの 4 ロールを broker（既定）または renga（opt-in 切り戻し）上で連携させ、個人〜小規模チームでの運用テンプレートを提供することを主旨としています。
