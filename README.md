...
そしてもう 1 つの軸が **安全寄りの設定のまま日常運用できること** です。Claude Code は厳しめのサンドボックス・細かい `permissions` 設定・PreToolUse フックを入れると確認が増えて使いにくくなり、逆に権限を広く開けて楽にすると事故が怖い、という either/or の罠があります。claude-org-ja はその面倒を **仕組みの側に持たせる** ことで両立を狙います: 窓口は人間との接点に専念して慎重に振る舞い、ワーカーはタスクごとの作業ディレクトリに閉じ込められ、ロールごとに `permissions.allow` / `permissions.deny` を分け、hooks と harness が破壊的操作を自動的に遮断し、状態を `.state/` に残して追跡可能にし、push / PR 作成 / スコープ拡張といった重要判断は必ず人間に戻します。**安全のための面倒を、人間ではなく仕組みに持たせる** — これが労働集約軸と並ぶ、もう 1 つの差別化軸です（詳細は [`docs/non-goals.md`](docs/non-goals.md) §1 と[セキュリティと許可境界](#セキュリティと許可境界)節を参照）。
