...
## これは何か
「ハーネスエンジニアリングやループエンジニアリングに興味はあるけれど、いきなり自分で構築するのは難しい」という人が最初の一歩として使うのに向いている Claude Code 組織運用ハーネスです。

claude-org-ja は、Claude Code を「人とコミュニケーションをとる窓口 1 つ + 裏で働くワーカー多数」の体制で長く動かすためのClaude Code組織ハーネスです。利用者は窓口の Claude Code 1つと会話するだけでタスクの分解、ワーカーへの作業割り当て、作業状態の保存と再開、たまった知見の整理は、すべて裏側で自動的に回ります。
たまった知見を元にスキルの修正や新たなスキルの作成を提案する機能も備えています。

人間の介入を極力減らし、タスクの選択とPush・PR作成、重要な設計判断以外はできるかぎり、エージェントに権限を移譲し自走させる為の設計です。

これらの機能を提供するために必要なセキュリティ設定も事前に組み込まれています。
