...
claude-org-ja は、Claude Code を「人間が話す窓口（Secretary）1 つ + 裏で働くワーカー多数」の体制で長時間動かすための運用層です。利用者は窓口の Claude 1 つと会話するだけでよく、起動・タスク分配・ワーカーの管理・状態の保存と再開はすべて裏側で自動的に回ります。

向いているのは、Claude Code を業務で長く回したい開発者・オペレーターのうち、全自動で暴走させるよりも「どこから先は人間の許可が要るか」という明示的な境界を引き、3〜5 人ぶんのワーカーを品質重視で運用したい層です。

- **最短で試す**: 1 行でインストールできるコマンドは [クイックスタート](#クイックスタート)にあります（ここでは重複させず、そちらへどうぞ）。
- **まず読む 3 つ**:
  - [`docs/getting-started.md`](docs/getting-started.md) — 導入・前提条件・手動での導入手順
  - [`docs/overview-business.md`](docs/overview-business.md) — 技術用語を避けた、業務目線のやさしい概要
  - README 内の [仕組み](#仕組み) — 役割分担と全体像の図
...
### 第一の柱: 全エージェント対話 TUI — headless を構造的に排除し安全機構を全タスクに効かせる

全エージェント（窓口・ディスパッチャー・キュレーター・ワーカー）は**対話型 TUI セッションのまま**動き、`claude -p` / Agent SDK といった headless 実行を一切使いません。これは単なる起動方法の選択ではなく、安全モデルの土台です。sandbox / PreToolUse フック / 許可境界（permissions）といった安全機構は対話セッションの実行経路に結び付いており、全エージェントが対話 TUI で動くことで、これらの防御が**例外なく全タスクへ一貫適用**されます。headless の抜け道で安全機構を回避する実行経路が、そもそも存在しません。headless を前提にしたエージェント farm / ループ構成との最大の差がここにあります。
...
Loop Engineering[^loop] は「エージェントに毎回プロンプトを与える（Prompt Engineering）」から「エージェントが次の手を自分で決めて回り続ける**ループそのもの**を設計する」への発想転換です。設計対象はハーネス（プロンプト / ルール / ツール）の一段上、すなわち**単発のやり取りではなく、何百回繰り返しても精度が劣化しない状態**になります。claude-org-ja は、この考え方が説く 6 要素を実装に落とし込んだリファレンスです。

| 要素 | 本システムでの実装 |
|---|---|
| **Trigger**（起動契機） | peer message 受信（renga=in-band push / broker=push 一次 + pull フォールバック）/ PR マージ後の next-dispatch / ディスパッチャーの定期監視 |
| **Context**（文脈供給） | `CLAUDE.md` ベースライン + `.state/` + handover/resume |
| **Action**（実行） | 作業ディレクトリ境界の中でのワーカー実作業 |
| **Verification**（検証） | テスト / lint / 型チェック + CI 監視 + 人間ゲート。さらに任意で **`codex` による別モデルの独立レビュー**を組み込めます。「self-review ではなく独立した検証役を置け」という Loop Engineering の指摘どおり、検証は実装したエージェント自身ではなく別プロセス・別モデルに担わせられる設計です |
| **Memory**（記憶） | 生の知見 → 整理済み知見への昇華パイプライン（キュレーター）+ handover/resume。「長時間ループのボトルネックは知能でもツールでもプロンプトでもなく記憶だ」（mem0 の 2026-06 分析）という指摘への直接の回答として、セッションをまたいでも劣化しない記憶を設計しています |
| **Escalation**（判断委譲） | 判断仰ぎの人間集約 + pending-decisions 台帳 |

ただし 6 要素を埋めるだけでは足りません。Loop Engineering の本質は次の**設計原則**にあります。

- **検証可能なゴール + 停止条件 + ガードレール**: 「全テスト通過 / build が exit 0 / キューが空」のように機械が完了を判定できるゴールでなければ、ループは暴走し無駄なトークンを焼きます。本システムでは CI green / テスト通過を機械判定ゴール、許可境界（permissions）をガードレール、人間ゲートを停止条件として対応づけています。
- **記憶が律速**: 上表 Memory のとおり、長時間ループの上限を決めるのは知能でもツールでもなく記憶です。生→整理の昇華パイプラインと handover/resume が、セッション境界での文脈消失を防ぎます。
- **独立検証**: self-review に頼らず、別モデル（`codex`）による検証を Verification に組み込めます。

さらに Loop Engineering は、ループが優秀になるほど中を覗かなくなる "Comprehension Debt"、"AI がそう判断したなら正しい" と委ねてしまう "Cognitive Surrender" という**堕落の罠**（Addy Osmani）を警告します。理解と意図を手放した設計者は、エンジニアではなく管理人に転落します。claude-org-ja が人間に判断だけを供給させ、escalation・attention layer・pending-decisions 台帳で取りこぼしを防ぐのは、まさにこの転落に抗して人間を判断ループの内側に留めるためです（次の第三の柱）。

加えて、提唱の整理には無い**ループ死活監視**（無音停止・リレー欠落の検出）・**引き継ぎ機構**（context 肥大時の handover → resume）・**人間ゲートのスロットル運用**（承認待ちの能動通知と台帳化）まで上乗せしています。
...
> **輸送層は両系（既定 renga / opt-in broker）**: ja の既定 transport は **renga** のままです。runtime 内蔵の **broker**（`org-broker`）は `ORG_TRANSPORT=broker` で opt-in でき、現在は dogfood 段階にあります。既定 renga は opt-in fallback として常時有効で、いつでも切戻し可能です。broker の起動・ライフサイクル・headless 排除（対話 TUI）attestation・切戻し手順は [`docs/operations/broker-dogfood-runbook.md`](docs/operations/broker-dogfood-runbook.md) を参照してください。
...
| **ヘッドレス型エージェントループ（`claude -p` / Agent SDK farm）** | エージェントを headless 実行で大規模並列・自動ループさせる構成 | claude-org-ja は**全エージェントが対話型 TUI** で動き headless 実行を構造的に排除する → sandbox / フック / 許可境界が全タスクに一貫適用される（詳細は第一の柱を参照） |
...
- **キュレーター（Curator）— 知見が勝手に資産化される**: 生の知見が閾値を超えた worker クローズ時にディスパッチャーがオンデマンド起動し、整理済み知見への昇華とスキル / プロセス改善提案を 1 回実行して自動クローズする
- **ワーカー（Worker）— 実作業はここに閉じ込める**: タスクごとの作業ディレクトリ境界の中で自律的にコミットまで行い（プルリクエスト作成は窓口側）、完了後に生の知見を記録する

全ペインは同一タブ内で動作します（別タブを開く `new_tab` は組織運用では使いません）。これは既定 transport の renga 前提で、opt-in の broker では detached 運用も可。
...
スキルは prefix で 3 系統に分かれます。`/org-*` は日々の組織運用（起動・派遣・中断・振り返り）、`/secretary-*` は組織を止めずに窓口だけ refresh する context 引き継ぎ、`/skill-*` はスキル体系自体のメタ操作（生成 → 棚卸し）です。新しくスキルを追加するときもこの prefix 規則に従います。
...
生成（eligibility-check）→ 棚卸し（audit）の順で自己成長ループを成す。
