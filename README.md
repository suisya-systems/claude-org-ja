...
つまり **「複数 Claude Code をどう束ねるか」というメタな労働集約自体を、人間から AI（窓口）に移譲する** のが claude-org-ja の最大の差別化軸です。Agent View が「人間がより速く見渡せるようにする」道具なのに対し、claude-org-ja は「人間が見渡さなくて済むようにする」運用層と言い換えてもよいでしょう。共存可能で、Agent View 配下に窓口を据えて使うこともできます。
...
|---|---|---|
| **Claude Code Agent View（公式）** | 複数 Claude Code を 1 画面で見渡すための**可視化機能** | claude-org-ja は可視化ではなく**労働集約自体の移譲**。起動・指示・分配の判断を窓口 AI に渡す。Agent View 配下で claude-org-ja を動かす形でも共存可能 |
