...
そしてもう 1 つの軸が **安全寄りの設定のまま日常運用できること** です。Claude Code には ① `permissions.defaultMode`（`default` は都度確認 / `auto` は分類器判定 / `bypassPermissions` は確認なしだが `permissions.allow` / `permissions.deny` を bypass）、② 静的な `permissions.allow` / `permissions.deny`、③ Bash サブプロセスに OS 層で効く `sandbox.filesystem.denyRead` / `denyWrite`（macOS / Linux / WSL2 のみで、Read tool 経由は別途 `Read(...)` deny が必要）、④ 動的に走り `bypassPermissions` 下でも残る `PreToolUse` フックの **4 つの層** があります。単独だと穴が残り、例えば `default` は確認疲れで日常運用に向かず、`bypassPermissions` を全ロールに撒くと `git push --force` 等の `permissions.deny` までまとめて無効になり、`sandbox.filesystem` は Windows native では未実装で素通りし、フックも関数定義経由の迂回までは捕捉しません。claude-org-ja はこの 4 層を **ロール別に組み合わせて配備** することで両立を狙います: 窓口・キュレーター・ワーカーは `auto` モードに narrow な `permissions.allow` + ロール別フック（ワーカーは `check-worker-boundary.sh` で作業ディレクトリ境界、窓口は `block-dangerous-git.sh` / `block-no-verify.sh` 等）を重ね、ディスパッチャーは Sonnet 環境で `auto` が選べないため `bypassPermissions` 固定としつつ `block-dispatcher-out-of-scope.sh` で Edit/Write を `.dispatcher/` / `.state/` / `knowledge/raw/` に限定し、個人環境では `python tools/org_setup_prune.py --user-common-sandbox` が `~/.claude/settings.json` の sandbox `denyRead` / `denyWrite` を idempotent に補強し、push / PR 作成といった重要判断は必ず人間に戻します。**安全のための面倒を、人間ではなく仕組みに持たせる** — これが労働集約軸と並ぶ、もう 1 つの差別化軸です（4 層 × 攻撃ベクトルの対応表は [`docs/verification.md` §12 攻撃ベクトル × 防御層マトリクス](docs/verification.md#security-matrix)、ディスパッチャー bypass 例外と `bypassPermissions` 下での PreToolUse フックの実効防御は [`docs/non-goals.md` §1](docs/non-goals.md#1-ワーカーに---dangerously-skip-permissions-を既定で撒かない) を参照）。
