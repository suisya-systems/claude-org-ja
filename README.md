...
- on-disk 検証の既定挙動: `.claude/settings.local.json` / `.foreman/.claude/settings.local.json` / `.curator/.claude/settings.local.json` はいずれも `.gitignore` 対象であり、default の `python tools/check_role_configs.py` は git 管理下のファイル（`.claude/settings.json` など）のみ強制する。ローカル worktree の実設定も一括監査したい場合は `--include-local` を付けて実行する（ただし worker worktree 内では `.claude/settings.local.json` が secretary スキーマと形状が異なるため、role 的に該当する worktree から実行すること）。
