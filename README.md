...
- **既知の false positive**: 同一セグメント内のリテラル文字列も loose match で拾う（例: `echo git commit --no-verify` は実際には実行されないが拒否される）。多層防御の最後の壁としては誤検知を許容する設計（精緻な tokenizer 化は Phase 2 で再評価）。
